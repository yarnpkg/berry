...
const IGNORE_CONSISTENT_DEPENDENCIES_FOR = new Set([`.`, `packages/docusaurus`]);
...
function enforceConsistentDependenciesAcrossTheProject({ Yarn }) {
  for (const dependency of Yarn.dependencies()) {
    if (IGNORE_CONSISTENT_DEPENDENCIES_FOR.has(dependency.workspace.cwd)) continue;
...
function enforceUpdateLocalScripts({ Yarn }) {
  ...
  if (!cli) throw new Error(`Assertion failed: We need the @yarnpkg/cli workspace to be present`);

  for (const workspace of Yarn.workspaces()) {
    if (!workspace.ident?.startsWith(`@yarnpkg/plugin-`)) continue;

    if (cli.manifest[`@yarnpkg/builder`].bundles.standard.includes(workspace.ident)) continue;
