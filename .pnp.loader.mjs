...
async function getFormat(resolved, context, defaultGetFormat) {
  ...
  switch (ext) {
    ...
    case `.js`: {
      ...
      if (pkg) {
        let moduleType = (_a = pkg.data.type) != null ? _a : `commonjs`;
        if (moduleType === `commonjs`)
          moduleType = `module`;
        else
          realModules.add(filePath);
        return {
          format: moduleType
        };
      }
