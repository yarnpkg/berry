  ...
  class extends Command {
    ...
    async execute() {
      xfs.mktempSync((temp) => {
        ...
        xfs.symlinkSync(
          ppath.join(repoDir, `packages/yarnpkg-cli/bundles/yarn.js`),
          ppath.join(binaryDir, `yarn-after`),
        );
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `yarn-repo`),
          [
            `#!/bin/bash\n`,
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `bench-commit`),
          [
            `#!/bin/bash\n`,
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `bench-import`),
          [
            `#!/bin/bash\n`,
            `cd ${npath.fromPortablePath(temp)}\n`,
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `bench-reset`),
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `bench-run`),
          [
            `#!/bin/bash\n`,
            `cd ${npath.fromPortablePath(temp)}\n`,
            `\n`,
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `hyperfine/before`),
          [
            `#!/bin/bash\n`,
            `cd ${npath.fromPortablePath(temp)}\n`,
        ...
        xfs.writeFileSync(
          ppath.join(binaryDir, `hyperfine/after`),
          [
            `#!/bin/bash\n`,
            `cd ${npath.fromPortablePath(temp)}\n`,
        ...
        xfs.moveSync(
          ppath.join(releaseFolder, xfs.readdirSync(releaseFolder)[0]),
          ppath.join(binaryDir, `yarn-before`),
        );
        ...
        console.log();
        console.log(
          `  - Setup the initial state of your repository, then run ${chalk.magenta(`bench-commit`)} to persist it in the temporary repository.`,
        );
        console.log(
          `    (note that this repository intentionally lacks a gitignore - feel free to commit archives, metadata, etc)`,
        );
        console.log(
          `  - If you want to run some code before the benchmark, add it to the ${chalk.yellow(`bench-prepare.sh`)} script in this folder.`,
        );
        console.log(
          `  - By default the benchmark will run ${chalk.magenta(`yarn install`)}; you can change that by editing ${chalk.yellow(`bench-script.sh`)}.`,
        );
        console.log(
          `  - When you want to run the benchmark, run ${chalk.magenta(`bench-run`)}. The repository will be reset between each run.`,
        );
        console.log(
          `  - If using OSX or Linux, the results will be automatically copied to your clipboard. Otherwise, they'll be available in ${chalk.yellow(`.git/yarn-bench`)}.`,
        );
        console.log();
