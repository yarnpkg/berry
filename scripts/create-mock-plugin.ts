  ...
  class extends Command {
    ...
    async execute() {
      ...
      await xfs.writeFilePromise(
        bundleLocation,
        `module.exports = {name: ${JSON.stringify(`@yarnpkg/${identifier}`)}};`,
      );

      this.context.stdout.write(`This plugin is now a builtin and can be safely removed from your configuration.\n`);
      this.context.stdout.write(
        `In the meantime, we'll replace it by an empty plugin; you'll need to re-import those if you decide later to go back to an older release.\n`,
      );
    }
  },
);
