...
async function execute(files: Array<string>) {
  ...
  for (const relativePath of files) {
    ...
    for (const hook of fileHooks) {
      ...
      if (existingDefinition.definition !== hook.definition)
        throw new Error(`Mismatched hook definitions for ${hook.name}`);

      if (typeof hook.comment !== `undefined`) {
        if (typeof existingDefinition.file !== `undefined`)
          if (existingDefinition.file !== hook.file)
            throw new Error(`Duplicate hook documentation; only document it in one file`);
  ...
  for (const hook of allHooksArray)
    if (typeof hook.comment === `undefined`)
      throw new Error(`Undocumented hook ${hook.name}`);

  return allHooksArray as Array<Required<HookDefinition>>;
}

module.exports = async () => {
  const repositoryDirectory = path.resolve(__dirname, `..`);
  const packageDirectory = path.join(repositoryDirectory, `packages`);
  ...
  indexList.unshift(path.join(packageDirectory, `yarnpkg-core/sources/Plugin.ts`));

  const hooks = await execute(indexList);

  return hooks.map(hook => [
    `### \`${hook.name}\``,
    `Defined in [${path.relative(repositoryDirectory, hook.file)}](https://github.com/yarnpkg/berry/blob/master/${path.relative(repositoryDirectory, hook.file)})`,
    ``,
    hook.comment,
    ``,
    `\`\`\`ts twoslash`,
    ...
    `interface Hook {`,
    `${hook.definition.replace(/,$/, `;`).replace(/^/gm, `  `)}`,
    `}`,
    `\`\`\``,
  ].join(`\n`)).join(`\n\n`);
};
