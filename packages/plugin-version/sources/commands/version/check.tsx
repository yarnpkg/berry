...
export default class VersionCheckCommand extends BaseCommand {
  ...
  async executeInteractive() {
    ...
    if (versionFile === null || versionFile.releaseRoots.size === 0) return 0;

    if (versionFile.root === null) throw new UsageError(`This command can only be run on Git repositories`);
          ...
          <Box flexDirection={`column`} width={60}>
            ...
            <Box>
              <Text>
                ...
                <Text bold color={`cyanBright`}>{`<right>`}</Text> to select release strategies.
              </Text>
            </Box>
          </Box>
    ...
    const Undecided = ({
      ...
      if (currentVersion === null)
        throw new Error(
          `Assertion failed: The version should have been set (${structUtils.prettyLocator(configuration, workspace.anchoredLocator)})`,
        );
    ...
    const useReleases = (): [
      ...
      const setWorkspaceRelease = useCallback(
        (workspace: Workspace, decision: versionUtils.Decision) => {
          ...
          if (decision !== versionUtils.Decision.UNDECIDED) copy.set(workspace, decision);
          else copy.delete(workspace);
        ...
        <Box flexDirection={`column`}>
            ...
            <>
              <Box marginTop={1}>
                <Text wrap={`wrap`}>
                  ...
                  (note that private workspaces are also shown here, because even though they won't be published,
          ...
          {dependentWorkspaces.size > 0 ? (
            <>
              ...
              <Box>
                <Text>
                  (Press <Text bold color={`cyanBright`}>{`<tab>`}</Text> to move the focus between the workspace
                  groups.)
                </Text>
              </Box>
  ...
  async executeStandard() {
    ...
    const report = await StreamReport.start(
      ...
      async (report) => {
        ...
        report.reportInfo(
          MessageName.UNNAMED,
          `Your PR was started right after ${formatUtils.pretty(configuration, versionFile.baseHash.slice(0, 7), `yellow`)} ${formatUtils.pretty(configuration, versionFile.baseTitle, `magenta`)}`,
        );
        ...
        if (versionFile.changedFiles.size > 0) {
          ...
          for (const file of versionFile.changedFiles) {
            report.reportInfo(
              null,
              `${formatUtils.pretty(configuration, npath.fromPortablePath(versionFile.root), `gray`)}${npath.sep}${npath.relative(npath.fromPortablePath(versionFile.root), npath.fromPortablePath(file))}`,
            );
          }
        }
        ...
        if (undecided.size > 0) {
          ...
          for (const workspace of undecided)
            report.reportError(
              MessageName.UNNAMED,
              `${structUtils.prettyLocator(configuration, workspace.anchoredLocator)} has been modified but doesn't have a release strategy attached`,
            );
        ...
        for (const [workspace, dependency] of undecidedDependents) {
          ...
          report.reportError(
            MessageName.UNNAMED,
            `${structUtils.prettyLocator(configuration, workspace.anchoredLocator)} doesn't have a release strategy attached, but depends on ${structUtils.prettyWorkspace(configuration, dependency)} which is planned for release.`,
          );
          hasDepsErrors = true;
        }
        ...
        if (hasDiffErrors || hasDepsErrors) {
          ...
          report.reportInfo(
            MessageName.UNNAMED,
            `This command detected that at least some workspaces have received modifications without explicit instructions as to how they had to be released (if needed).`,
          );
          report.reportInfo(
            MessageName.UNNAMED,
            `To correct these errors, run \`yarn version check --interactive\` then follow the instructions.`,
          );
        }
      },
    );
