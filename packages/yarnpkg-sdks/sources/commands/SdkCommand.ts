...
export default class SdkCommand extends Command {
  ...
  static usage = Command.Usage({
    ...
    `,
    examples: [
      [`Generate the base SDKs`, `$0 base`],
      [`Generate the base SDKs and editor settings for supported editors`, `$0 vscode vim`],
      [`Update all generated SDKs and editor settings`, `$0`],
    ],
  });
  ...
  async execute() {
    ...
    const requestedIntegrations =
      this.integrations.length === 0 || onlyBase
    ...
    const preexistingIntegrations =
      integrationsFile !== null ? integrationsFile.integrations : new Set<SupportedIntegration>();

    const allIntegrations = new Set([...requestedIntegrations, ...preexistingIntegrations]);

    if (allIntegrations.size === 0 && !onlyBase)
      throw new UsageError(
        `No integrations have been provided as arguments, and no preexisting integrations could be found. Make sure to run \`yarn sdks <integrations>\` first, or \`yarn sdks base\` if you only need the SDK files and prefer to manage your own environment settings. Run \`yarn sdks -h\` to see the list of supported integrations.`,
      );
