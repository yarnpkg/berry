...
export const validateIntegrations = (integrations: Set<string>) => {
  ...
  if (unsupportedIntegrations.length > 0) {
    throw new UsageError(
      `No supported integrations with the following names could be found: ${unsupportedIntegrations.join(`, `)}. Run \`yarn sdks -h\` to see the list of supported integrations.`,
    );
  }
};
...
export class IntegrationsFile {
  ...
  static async find(projectRoot: PortablePath) {
    ...
    if (!xfs.existsSync(integrationPath)) return null;
  ...
  load(data: any) {
    if (typeof data !== `object` || data === null) throw new Error(`Utterly invalid integrations file data (${data})`);
  ...
  exportTo(data: { [key: string]: any }) {
    if (this.integrations.size > 0) data.integrations = [...this.integrations];
...
const TEMPLATE = (
  relPnpApiPath: PortablePath,
  module: string,
  { setupEnv = false, usePnpify = false, wrapModule }: TemplateOptions,
) =>
  [
    `#!/usr/bin/env node\n`,
    `\n`,
...
export type IntegrationSdks = Array<
  [null, GenerateDefaultWrapper | null] | [SupportedSdk, GenerateIntegrationWrapper | null]
>;

export type PackageExports = { [key: string]: PackageExports } | Array<PackageExports> | string | null;
...
export class Wrapper {
  ...
  constructor(
    name: PortablePath,
    {
      ...
      target,
      manifestOverrides = {},
    }: { pnpApi: PnpApi; target: PortablePath; manifestOverrides?: Record<string, any> },
  ) {
    this.name = name;
  ...
  async writeDefaults() {
    ...
    if (this.manifest.exports) await this.writePackageExports();

    if (this.manifest.bin) await this.writePackageBinaries();
  ...
  async writePackageExports(packageExports: PackageExports = this.manifest.exports, requirePath = PortablePath.dot) {
    if (typeof packageExports === `string`) {
      ...
      await Promise.all(packageExports.map((packageExport) => this.writePackageExports(packageExport, requirePath)));
  ...
  async writeFile(
    relPackagePath: PortablePath,
    options: TemplateOptions & { requirePath?: PortablePath; mode?: number } = {},
  ) {
    const topLevelInformation = this.pnpApi.getPackageInformation(this.pnpApi.topLevel)!;
    const projectRoot = npath.toPortablePath(topLevelInformation.packageLocation);
...
export const generateSdk = async (
  pnpApi: PnpApi,
  { requestedIntegrations, preexistingIntegrations }: AllIntegrations,
  {
    report,
    onlyBase,
    verbose,
    configuration,
  }: { report: Report; onlyBase: boolean; verbose: boolean; configuration: Configuration },
): Promise<void> => {
  ...
  const integrationSdks = miscUtils.mapAndFilter(SUPPORTED_INTEGRATIONS, ([integration, sdk]) => {
    if (!allIntegrations.has(integration)) return miscUtils.mapAndFilter.skip;

    return sdk;
  });

  await report.startTimerPromise(
    `Generating SDKs inside ${formatUtils.pretty(configuration, SDK_FOLDER, formatUtils.Type.PATH)}`,
    async () => {
      const skipped = [];

      for (const sdks of integrationSdks) {
        const defaultSdk = sdks.find((sdk) => sdk[0] === null);
        if (!defaultSdk) continue;
      ...
      for (const [pkgName, generateBaseWrapper] of BASE_SDKS) {
        ...
        if (topLevelInformation.packageDependencies.has(pkgName)) {
          ...
          for (const sdks of integrationSdks) {
            ...
            if (!generateIntegrationWrapper) continue;
        ...
        if (verbose) {
          for (const displayName of skipped) {
            report.reportWarning(
              MessageName.UNNAMED,
              `${chalk.yellow(`•`)} ${displayName} (dependency not found; skipped)`,
            );
          }
        } else {
          report.reportWarning(
            MessageName.UNNAMED,
            `${chalk.yellow(`•`)} ${skipped.length} SDKs were skipped based on your root dependencies`,
          );
        }
      }
    },
