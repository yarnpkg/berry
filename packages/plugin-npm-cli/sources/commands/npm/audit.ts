...
export default class NpmAuditCommand extends BaseCommand {
  ...
  static usage: Usage = Command.Usage({
    ...
    examples: [
      ...
      [`Limit auditing to \`dependencies\` (excludes \`devDependencies\`)`, `yarn npm audit --environment production`],
      ...
      [`Exclude certain packages`, `yarn npm audit --exclude package1 --exclude package2`],
      [`Ignore specific advisories`, `yarn npm audit --ignore 1234567 --ignore 7654321`],
    ],
  });
  ...
  async execute() {
    ...
    const topLevelDependencies = npmAuditUtils.getTopLevelDependencies(project, workspace, {
      all: this.all,
      environment: this.environment,
    });
    const packages = npmAuditUtils.getPackages(project, topLevelDependencies, { recursive: this.recursive });

    const excludedPackages = Array.from(new Set([...configuration.get(`npmAuditExcludePackages`), ...this.excludes]));
    ...
    const httpReport = await LightReport.start(
      ...
      async () => {
        ...
        const deprecations = this.noDeprecations
          ? []
          : await Promise.all(
              Array.from(Object.entries(payload), async ([packageName, versions]) => {
                ...
                return miscUtils.mapAndFilter(versions, (version) => {
                  ...
                  if (!deprecated) return miscUtils.mapAndFilter.skip;
        ...
        for (const [packageName, version, message] of deprecations.flat(1)) {
          ...
          if (Object.hasOwn(auditResult, packageName))
            if (
              auditResult[packageName].some((advisory) =>
                semverUtils.satisfiesWithPrereleases(version, advisory.vulnerable_versions),
              )
            )
              continue;
    ...
    const ignoredAdvisories = Array.from(new Set([...configuration.get(`npmAuditIgnoreAdvisories`), ...this.ignores]));
      ...
      if (filteredAdvisories.length > 0) {
        expandedResult[packageName] = filteredAdvisories.map((advisory) => {
          ...
          for (const version of versions)
            for (const pkg of packageVersions.get(version)!) dependents.set(pkg.locatorHash, pkg);
