...
export default class NpmTagRemoveCommand extends BaseCommand {
  ...
  async execute() {
    if (this.tag === `latest`) throw new UsageError(`The 'latest' tag cannot be removed.`);
    ...
    if (!Object.hasOwn(distTags, this.tag)) throw new UsageError(`${prettyTag} is not a tag of package ${prettyIdent}`);
