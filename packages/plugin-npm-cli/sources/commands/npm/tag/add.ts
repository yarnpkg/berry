  ...
  async execute() {
    ...
    if (!semver.valid(version))
      throw new UsageError(
        `The range ${formatUtils.pretty(configuration, descriptor.range, formatUtils.Type.RANGE)} must be a valid semver version`,
      );
    ...
    const report = await StreamReport.start(
      ...
      async (report) => {
        ...
        report.reportInfo(
          MessageName.UNNAMED,
          `Tag ${prettyTag} added to version ${prettyVersion} of package ${prettyIdent}`,
        );
      },
    );
