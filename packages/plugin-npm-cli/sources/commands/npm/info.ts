...
export default class NpmInfoCommand extends BaseCommand {
  ...
  static usage: Usage = Command.Usage({
    ...
    examples: [
      ...
      [`Show all available information about react@next`, `yarn npm info react@next`],
          ...
          if (identStr === `.`) {
            ...
            if (!workspace.manifest.name)
              throw new UsageError(
                `Missing ${formatUtils.pretty(configuration, `name`, formatUtils.Type.CODE)} field in ${npath.fromPortablePath(ppath.join(workspace.cwd, Filename.manifest))}`,
              );
          ...
          if (validRange) {
            ...
            if (maxSatisfyingVersion !== null) {
              ...
              report.reportWarning(
                MessageName.UNNAMED,
                `Unmet range ${structUtils.prettyRange(configuration, descriptor.range)}; falling back to the latest version`,
              );
            ...
            report.reportWarning(
              MessageName.UNNAMED,
              `Unknown tag ${structUtils.prettyRange(configuration, descriptor.range)}; falling back to the latest version`,
            );
            leadWithSeparator = true;
          }
            ...
            for (const field of fields) {
              ...
              if (typeof value !== `undefined`) {
                ...
                report.reportWarning(
                  ...
                  `The ${formatUtils.pretty(configuration, field, formatUtils.Type.CODE)} field doesn't exist inside ${structUtils.prettyIdent(configuration, descriptor)}'s information`,
                );
                leadWithSeparator = true;
                continue;
              }
