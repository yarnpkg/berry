...
export class ZipOpenFS extends FakeFS {
  ...
  private async makeCallPromise<T>(
    p: string,
    discard: () => Promise<T>,
    accept: (zipFS: ZipFS, zipInfo: {archivePath: string; subPath: string}) => Promise<T>,
  ...
  private makeCallSync<T>(
    p: string,
    discard: () => T,
    accept: (zipFS: ZipFS, zipInfo: {archivePath: string; subPath: string}) => T,
    {requireSubpath = true}: {requireSubpath?: boolean} = {},
  ): T {
    p = posix.normalize(posix.resolve(`/`, p));
  ...
  private findZip(p: string) {
    ...
    for (let t = 2; t <= parts.length; ++t) {
      ...
      if (this.isZip.has(archivePath)) return {archivePath, subPath: posix.resolve(`/`, parts.slice(t).join(`/`))};
    ...
    if (this.zipInstances) {
      ...
      if (!zipFs)
        this.zipInstances.set(
          p,
          (zipFs = new ZipFS(p, {baseFs: this.baseFs, stats: await this.baseFs.statPromise(p)})),
        );
  ...
  private getZipSync<T>(p: string, accept: (zipFs: ZipFS) => T) {
    if (this.zipInstances) {
      ...
      if (!zipFs) this.zipInstances.set(p, (zipFs = new ZipFS(p, {baseFs: this.baseFs})));
