...
function getFileDescriptorStream(fd: number, type: StreamType, state: ShellState) {
  ...
  switch (fd) {
    case Pipe.STDIN:
      {
        if ((type & StreamType.Readable) === StreamType.Readable) state.stdin.pipe(stream, { end: false });
    ...
    case Pipe.STDOUT:
      {
        if ((type & StreamType.Readable) === StreamType.Readable) state.stdout.pipe(stream, { end: false });
    ...
    case Pipe.STDERR:
      {
        if ((type & StreamType.Readable) === StreamType.Readable) state.stderr.pipe(stream, { end: false });
...
const BUILTINS = new Map<string, ShellBuiltin>([
  [
    `cd`,
    async ([target = homedir(), ...rest]: Array<string>, opts: ShellOptions, state: ShellState) => {
      ...
      const stat = await opts.baseFs.statPromise(resolvedTarget).catch((error) => {
        throw error.code === `ENOENT` ? new ShellError(`cd: no such file or directory: ${target}`) : error;
      });

      if (!stat.isDirectory()) throw new ShellError(`cd: not a directory: ${target}`);
  ...
  [
    `exit`,
    async ([code, ...rest]: Array<string>, opts: ShellOptions, state: ShellState) => {
  ...
  [
    `sleep`,
    async ([time]: Array<string>, opts: ShellOptions, state: ShellState) => {
      if (typeof time === `undefined`) throw new ShellError(`sleep: missing operand`);

      // TODO: make it support unit suffixes
      const seconds = Number(time);
      if (Number.isNaN(seconds)) throw new ShellError(`sleep: invalid time interval '${time}'`);
...
async function applyEnvVariables(environmentSegments: Array<EnvSegment>, opts: ShellOptions, state: ShellState) {
  ...
  return interpolatedEnvs.reduce(
    (envs, env) => {
...
async function evaluateVariable(
  segment: ArgumentSegment & { type: `variable` },
  opts: ShellOptions,
  state: ShellState,
    ...
    default:
      {
        ...
        if (typeof raw === `undefined`) {
          if (isArgument) throw new ShellError(`Unbound argument #${argIndex}`);
...
async function interpolateArguments(commandArgs: Array<Argument>, opts: ShellOptions, state: ShellState) {
  ...
  const redirect = (type: string, fd: number | null, target: string) => {
    ...
    if (typeof targets === `undefined`) redirections.set(key, (targets = []));
...
function makeActionFromProcedure(
  procedure: ProcessImplementation,
  args: Array<string>,
  opts: ShellOptions,
  activeState: ShellState,
) {
  if (args.length === 0) {
    return procedure;
  } else {
...
async function executeCommandLine(
  ...
  const executeChain = async (line: CommandLine) => {
    try {
      return await executeCommandChain(line.chain, opts, state, {
        background: background && typeof line.then === `undefined`,
      });
    } catch (error) {
  ...
  switch (segment.type) {
    case `variable`: {
      return (
        ...
        segment.name === `#` ||
        segment.name === `*` ||
        Number.isFinite(parseInt(segment.name, 10)) ||
        (`defaultValue` in segment &&
          !!segment.defaultValue &&
          segment.defaultValue.some((arg) => locateArgsVariableInArgument(arg))) ||
        (`alternativeValue` in segment &&
          !!segment.alternativeValue &&
          segment.alternativeValue.some((arg) => locateArgsVariableInArgument(arg)))
      );
    }
    case `arithmetic`: {
...
export async function execute(
  ...
  for (const [key, builtin] of Object.entries(builtins)) normalizedBuiltins.set(key, builtin);
