...
const ifNotWin32It = isNotWin32 ? it : it.skip;
...
const bufferResult = async (
  command: string,
  args: Array<string> = [],
  options: Partial<UserOptions> & { tty?: boolean } = {},
): Promise<Result> => {
  ...
  const exitCode = await execute(command, args, {
    ...options,
    stdout,
    stderr,
    builtins: {
      ...
      [`echo-arguments`]: async (args, opts, state) => {
        return await new Promise((resolve) => {
          for (const arg of args) state.stdout.write(`${JSON.stringify(arg)}\n`);
...
describe(`Shell`, () => {
  describe(`Simple shell features`, () => {
    describe(`Empty commands`, () => {
      ...
      it(`should support an empty string`, async () => {
        await expectResult(bufferResult(``), EMPTY_COMMAND_RESULT);
      });
      ...
      it(`should support an empty string when passing arguments`, async () => {
        await expectResult(bufferResult(``, [`hello`, `world`]), EMPTY_COMMAND_RESULT);
      });
    ...
    it(`should execute a regular command`, async () => {
      await expectResult(bufferResult(`echo hello`), {
        stdout: `hello\n`,
      });
    });

    it(`should support empty string as argument`, async () => {
      await expectResult(bufferResult(`node -pe "process.argv[2]" "" 1`), {
        stdout: `1\n`,
      });
    });
    ...
    ifNotWin32It(`should throw an error when a command doesn't exist`, async () => {
      await expectResult(bufferResult(`this-command-doesnt-exist-sorry`), {
        exitCode: 127,
    ...
    it(`should forward the specified exit code when running exit`, async () => {
      await expectResult(bufferResult(`exit 1`), {
        exitCode: 1,
      });

      await expectResult(bufferResult(`exit 42`), {
        exitCode: 42,
      });
    });

    it(`should shortcut the right branch of a '||' when the left branch succeeds`, async () => {
      await expectResult(bufferResult(`true || echo failed`), {
        stdout: ``,
      });
    });

    it(`should shortcut the right branch of a '&&' when the left branch fails`, async () => {
      await expectResult(bufferResult(`false && echo failed`), {
        exitCode: 1,
        stdout: ``,
      });
    });
    ...
    it(`should execute the right branch of a '||' when the left branch fails`, async () => {
      await expectResult(bufferResult(`false || echo succeeds`), {
        stdout: `succeeds\n`,
      });
    });

    it(`should execute the right branch of a '&&' when the left branch succeeds`, async () => {
      await expectResult(bufferResult(`true && echo succeeds`), {
        stdout: `succeeds\n`,
      });
    });

    it(`should execute both branches regardless of their exit status when using ';'`, async () => {
      await expectResult(bufferResult(`echo foo; echo bar`), {
        stdout: `foo\nbar\n`,
      });
    });

    it(`should immediately stop the execution when calling 'exit'`, async () => {
      await expectResult(bufferResult(`echo hello; exit 1; echo world`), {
        exitCode: 1,
        stdout: `hello\n`,
      });

      await expectResult(bufferResult(`echo hello && exit 0 && echo world`), {
        exitCode: 0,
        stdout: `hello\n`,
      });
    });
    ...
    it(`should execute a subshell when using grouping parentheses (within '||', shortcutted)`, async () => {
      await expectResult(bufferResult(`true || (echo hello; echo world)`), {
        stdout: ``,
      });
    });

    it(`should execute a subshell when using grouping parentheses (within '||')`, async () => {
      await expectResult(bufferResult(`false || (echo hello; echo world)`), {
        stdout: `hello\nworld\n`,
      });
    });
    ...
    it(`should allow subshells to access the $? from the parent shell`, async () => {
      await expectResult(bufferResult(`false; (echo $?)`), {
        stdout: `1\n`,
      });
    });
    ...
    it(`should interpolate subshells with the proper split`, async () => {
      await expectResult(bufferResult(`echo $(echo foo)bar`), {
        stdout: `foobar\n`,
      });
    });

    it(`should interpolate subshells with the proper split`, async () => {
      await expectResult(bufferResult(`echo $(echo 'foo      bar')bar`), {
        stdout: `foo barbar\n`,
      });
    });
    ...
    it(`should support redirections on subshells (one command)`, async () => {
      await xfs.mktempPromise(async (tmpDir) => {
        ...
        await expectResult(bufferResult(`(echo "hello world") > "${file}"`), {
          stdout: ``,
        });
    ...
    it(`should support redirections on subshells (multiple commands)`, async () => {
      await xfs.mktempPromise(async (tmpDir) => {
        ...
        await expectResult(bufferResult(`(echo "hello world"; echo "goodbye world") > "${file}"`), {
          stdout: ``,
        });
    ...
    it(`shouldn't allow subshells to mutate the state of the parent shell`, async () => {
      await expectResult(bufferResult(`(FOO=hello); echo $FOO`), {
        exitCode: 1,
  ...
  describe(`Variables`, () => {
    describe(`Built-in variables`, () => {
      it(`should expose the previous exit code via $?`, async () => {
        await expectResult(bufferResult(`true; echo $?`), {
          stdout: `0\n`,
        });
      ...
      it(`should expose the number of arguments via $#`, async () => {
        await expectResult(bufferResult(`echo $#`), {
          stdout: `0\n`,
        });

        await expectResult(bufferResult(`echo $#`, [`hello`, `world`]), {
          stdout: `2\n`,
        });
      });

      it(`should expose individual arguments via $0, $1, ..., $n`, async () => {
        await expectResult(bufferResult(`echo $0`, [`hello`, `world`]), {
          stdout: `hello\n`,
        });

        await expectResult(bufferResult(`echo $1`, [`hello`, `world`]), {
          stdout: `world\n`,
        });
      });

      it(`should support argument spread via $@`, async () => {
        await expectResult(bufferResult(`node -p 'JSON.stringify(process.argv.slice(1))' "$@"`, [`hello`, `world`]), {
          stdout: `["hello","world"]\n`,
        });
      });
      ...
      it(`should expose the shell ppid via $PPID`, async () => {
        await expectResult(bufferResult(`echo $PPID`), {
          // The shell runs in the same process as the tests
          stdout: `${process.ppid}\n`,
        });
      });
      ...
      it(`should split variables when referenced outside of quotes`, async () => {
        await expectResult(bufferResult(`FOO="hello world"; echo-arguments $FOO`), {
          stdout: `"hello"\n"world"\n`,
        });
      });

      it(`shouldn't turn empty variables into arguments when referenced outside of quotes`, async () => {
        await expectResult(bufferResult(`FOO=""; echo-arguments $FOO`), {
          stdout: ``,
        });
      });
    ...
    describe(`Setting variables`, () => {
      describe(`Assignment prefix`, () => {
        it(`should set environment variables`, async () => {
          await expectResult(bufferResult(`PORT=1234 node -e 'process.stdout.write(process.env.PORT)'`), {
            stdout: `1234`,
          });
        });
        ...
        it(`env assignment prefix syntax shouldn't persist it to the environment`, async () => {
          await expectResult(bufferResult([`FOO=1`, `FOO=2 echo hello`, `echo $FOO`].join(` ; `)), {
            stdout: `hello\n1\n`,
          });

          await expectResult(
            bufferResult([`FOO=1`, `FOO=2 echo hello`, `node -e 'process.stdout.write(process.env.FOO)'`].join(` ; `)),
            {
              stdout: `hello\n1`,
            },
          );
      ...
      describe(`Assignment without command`, () => {
        it(`should support setting env variable without command`, async () => {
          await expectResult(
            bufferResult([`FOO=1`, `FOO=2`, `node -e 'process.stdout.write(process.env.FOO)'`].join(` ; `)),
            {
              stdout: `2`,
            },
          );
        ...
        it(`should support multiple env assignment without command`, async () => {
          await expectResult(bufferResult([`FOO=1 BAR=2`, `echo hello`, `echo $BAR`].join(` ; `)), {
            stdout: `hello\n2\n`,
          });
        });

        it(`should evaluate variables once before starting execution`, async () => {
          await expectResult(bufferResult([`FOO=1`, `FOO=2 echo $FOO`].join(` ; `)), {
            stdout: `1\n`,
          });
        });
      });
    ...
    describe(`Using variables`, () => {
      it(`should support using environment variables`, async () => {
        await expectResult(bufferResult(`echo $FOOBAR`, [], { env: { FOOBAR: `hello world` } }), {
          stdout: `hello world\n`,
        });
      });

      it(`should support default arguments via \${ARG:-...}`, async () => {
        await expectResult(bufferResult(`echo "\${DOESNT_EXIST:-hello world}"`), {
          stdout: `hello world\n`,
        });
      });
      ...
      it(`should support empty default arguments`, async () => {
        await expectResult(bufferResult(`echo "foo\${DOESNT_EXIST:-}bar"`), {
          stdout: `foobar\n`,
        });
      });

      it(`should support alternative arguments via \${ARG:+...}`, async () => {
        await expectResult(bufferResult(`echo "\${FOOBAR:+hello world}"`, [], { env: { FOOBAR: `goodbye world` } }), {
          stdout: `hello world\n`,
        });

        await expectResult(bufferResult(`echo "\${FOOBAR:+hello world}"`), {
          stdout: `\n`,
        });
      });

      it(`should support alternative arguments via \${N:+...}`, async () => {
        await expectResult(bufferResult(`echo "\${0:+hello world}"`, [`goodbye world`]), {
          stdout: `hello world\n`,
        });
      ...
      it(`should support alternative default arguments`, async () => {
        await expectResult(bufferResult(`echo "foo\${FOOBAR:+}bar"`, [], { env: { FOOBAR: `goodbye world` } }), {
          stdout: `foobar\n`,
        });
      });
      ...
      describe(`Errors`, () => {
        it(`should throw recoverable errors on unbound variables`, async () => {
          await expectResult(bufferResult(`echo $INEXISTENT && echo OK || echo KO`), {
            exitCode: 0,
            stdout: `KO\n`,
        ...
        it(`should throw recoverable errors on unbound arguments`, async () => {
          await expectResult(bufferResult(`echo $42 && echo OK || echo KO`), {
            exitCode: 0,
            stdout: `KO\n`,
  ...
  describe(`Redirections`, () => {
    describe(`<`, () => {
      it(`should support input redirections (file)`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`cat < "${file}"`), {
            stdout: `hello world\n`,
          });
        });
      });
      ...
      it(`should support input redirections to fd (file)`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`cat 0< "${file}"`), {
            stdout: `hello world\n`,
          });
        });
      });
      ...
      it(`should support multiple inputs`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`cat < "${file1}" < "${file2}"`), {
            stdout: `foo bar baz\nhello world\n`,
          });
        });
      });
      ...
      it(`should throw on input redirections to unsupported file descriptors`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expect(bufferResult(`cat 1< "${file}"`)).rejects.toThrowError(`Unsupported file descriptor: "1"`);

          await expect(bufferResult(`cat 2< "${file}"`)).rejects.toThrowError(`Unsupported file descriptor: "2"`);

          await expect(bufferResult(`cat 3< "${file}"`)).rejects.toThrowError(`Unsupported file descriptor: "3"`);
        });
      });
    });
    ...
    describe(`<<<`, () => {
      it(`should support input redirections (string)`, async () => {
        await expectResult(bufferResult(`cat <<< "hello world"`), {
          stdout: `hello world\n`,
        });
      });

      it(`should support input redirections to fd (string)`, async () => {
        await expectResult(bufferResult(`cat 0<<< "hello world"`), {
          stdout: `hello world\n`,
        });
      });
      ...
      it(`should throw on input redirections to unsupported file descriptors`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`cat 1<<< "hello world"`)).rejects.toThrowError(`Unsupported file descriptor: "1"`);

          await expect(bufferResult(`cat 2<<< "hello world"`)).rejects.toThrowError(`Unsupported file descriptor: "2"`);

          await expect(bufferResult(`cat 3<<< "hello world"`)).rejects.toThrowError(`Unsupported file descriptor: "3"`);
        });
      });
    });
    ...
    describe(`>`, () => {
      ...
      it(`shouldn't affect unrelated commands`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo "hello world" > "${file}"; echo foo`), {
            stdout: `foo\n`,
          });
      ...
      it(`shouldn't do weird stuff when piping a builtin redirection`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo "hello world" > "${file1}" | echo "foo bar" > "${file2}"; echo test`), {
            stdout: `test\n`,
          });
      ...
      it(`should support output redirections from fd (stdout)`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`node -e "console.log(\`foo\`), console.error(\`bar\`)" 1> "${file}"`), {
            stderr: `bar\n`,
          });
      ...
      it(`should support output redirections from fd (stderr)`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`node -e "console.log(\`foo\`), console.error(\`bar\`)" 2> "${file}"`), {
            stdout: `foo\n`,
          });
      ...
      it(`should support multiple outputs`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo "hello world" > "${file1}" > "${file2}"`), {
            stdout: ``,
          });
      ...
      it(`should throw on output redirections to inexistent folder`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`echo "hello world" > "inexistent-folder/file.txt"`)).rejects.toThrowError(
      ...
      it(`should throw on output redirections from unsupported file descriptors`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`echo "hello world" 0> /dev/null`)).rejects.toThrowError(
            `Unsupported file descriptor: "0"`,
          );
        });

        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`echo "hello world" 3> /dev/null`)).rejects.toThrowError(
            `Unsupported file descriptor: "3"`,
          );
        });
      });
    });
    ...
    describe(`>&`, () => {
      it(`should support implicit stdout redirections (file descriptor)`, async () => {
        await expectResult(bufferResult(`echo "hello world" >& 1`), {
          stdout: `hello world\n`,
        });

        await expectResult(bufferResult(`echo "hello world" >& 2`), {
          stderr: `hello world\n`,
        });
      });

      it(`should support stdout redirections (file descriptor)`, async () => {
        await expectResult(bufferResult(`echo "hello world" 1>& 1`), {
          stdout: `hello world\n`,
        });

        await expectResult(bufferResult(`echo "hello world" 1>& 2`), {
          stderr: `hello world\n`,
        });
      });

      it(`should support stderr redirections (file descriptor)`, async () => {
        await expectResult(bufferResult(`node -e "console.error(\`hello world\`)" 2>& 1`), {
          stdout: `hello world\n`,
        });

        await expectResult(bufferResult(`node -e "console.error(\`hello world\`)" 2>& 2`), {
          stderr: `hello world\n`,
        });
      });
      ...
      it(`should throw on output redirections from unsupported file descriptors`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`echo "hello world" 0>& 1`)).rejects.toThrowError(
            `Unsupported file descriptor: "0"`,
          );
        });

        await xfs.mktempPromise(async (tmpDir) => {
          await expect(bufferResult(`echo "hello world" 3>& 1`)).rejects.toThrowError(
            `Unsupported file descriptor: "3"`,
          );
        });
      });
      ...
      it(`should throw recoverable errors when a bad file descriptor is encountered`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expectResult(bufferResult(`echo "hello world" >& 42 && echo OK || echo KO`, [], { cwd: tmpDir }), {
            exitCode: 0,
            stdout: `KO\n`,
  ...
  describe(`Pipelines`, () => {
    describe(`|`, () => {
      ...
      it(`should pipe the result of a command into another (two commands, native into pipe)`, async () => {
        await expectResult(
          bufferResult([`node -e 'process.stdout.write("abcdefgh\\n");'`, `test-builtin`].join(` | `)),
          {
            stdout: `aceg\n`,
          },
        );
      ...
      it(`should pipe the result of a command into another (only builtins)`, async () => {
        await expectResult(bufferResult([`echo abcdefghijkl`, `test-builtin`, `test-builtin`].join(` | `)), {
          stdout: `aei\n`,
        });
      });

      it(`should pipe the stdout of a command into another`, async () => {
        await expectResult(bufferResult([`node -e 'process.stdout.write("Hello World");'`, `echo-stdin`].join(` | `)), {
          stdout: `Hello World\n`,
        });
      });

      it(`shouldn't pipe the stderr of a command into another`, async () => {
        await expectResult(bufferResult([`node -e 'process.stderr.write("Hello World");'`, `echo-stdin`].join(` | `)), {
          stdout: `\n`,
          stderr: `Hello World`,
        });
      });
    ...
    describe(`|&`, () => {
      it(`should pipe the stdout of a command into another`, async () => {
        await expectResult(
          bufferResult([`node -e 'process.stdout.write("Hello World");'`, `echo-stdin`].join(` |& `)),
          {
            stdout: `Hello World\n`,
          },
        );
      ...
      it(`should pipe the stderr of a command into another`, async () => {
        await expectResult(
          bufferResult([`node -e 'process.stderr.write("Hello World");'`, `echo-stdin`].join(` |& `)),
          {
            stdout: `Hello World\n`,
          },
        );
  ...
  describe(`Lists`, () => {
    it(`should execute lists with left associativity`, async () => {
      await expectResult(bufferResult(`false && echo yes || echo no`), {
        exitCode: 0,
        stdout: `no\n`,
        stderr: ``,
      });

      await expectResult(bufferResult(`false || echo no && echo yes`), {
        exitCode: 0,
        stdout: `no\nyes\n`,
        stderr: ``,
      });

      await expectResult(bufferResult(`false && echo yes || false && echo yes || echo no`), {
        exitCode: 0,
        stdout: `no\n`,
        stderr: ``,
      });
      ...
      describe(`Basic Syntax`, () => {
        it(`should support glob patterns with asterisk`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo *`, [], { cwd: tmpDir }), {
              stdout: `a.txt b.txt c.txt\n`,
            });

            await expectResult(bufferResult(`echo *.txt`, [], { cwd: tmpDir }), {
              stdout: `a.txt b.txt c.txt\n`,
            });
          });
        });
        ...
        it(`should support glob patterns with question mark`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo a?.txt`, [], { cwd: tmpDir }), {
              stdout: `ax.txt\n`,
            });

            await expectResult(bufferResult(`echo a??a.txt`, [], { cwd: tmpDir }), {
              stdout: `axxa.txt\n`,
            });
        ...
        it(`should support glob patterns with sequence`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo a[23].txt`, [], { cwd: tmpDir }), {
              stdout: `a2.txt a3.txt\n`,
            });

            await expectResult(bufferResult(`echo foo.[jt]s`, [], { cwd: tmpDir }), {
      ...
      describe(`Advanced Syntax`, () => {
        it(`should support glob patterns with posix character classes`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            if (process.platform !== `win32`) await xfs.writeFilePromise(ppath.join(tmpDir, `😀.txt`), ``);

            await expectResult(bufferResult(`echo +([[:alnum:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `123.txt BAR.txt abc.txt foo.txt foo123.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:alpha:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `BAR.txt abc.txt foo.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:ascii:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `&434)hello.txt 123.txt BAR.txt abc.txt foo.txt foo123.txt hello_world123.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:digit:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `123.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:lower:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `abc.txt foo.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:upper:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `BAR.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:word:]]).txt`, [], { cwd: tmpDir }), {
              stdout: `123.txt BAR.txt abc.txt foo.txt foo123.txt hello_world123.txt\n`,
            });

            await expectResult(bufferResult(`echo +([[:xdigit:]]).txt`, [], { cwd: tmpDir }), {
        ...
        it(`should support glob patterns with extglobs`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo f@(o).txt`, [], { cwd: tmpDir }), {
              stdout: `fo.txt\n`,
            });
        ...
        it(`should support glob patterns with braces`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo {a,b}.txt`, [], { cwd: tmpDir }), {
              stdout: `a.txt b.txt\n`,
            });

            await expectResult(bufferResult(`echo {cb,ft}{ax,st}{bc,wy}.txt`, [], { cwd: tmpDir }), {
              stdout: `cbaxbc.txt cbstbc.txt ftaxwy.txt\n`,
            });

            await expectResult(bufferResult(`echo a{bc,{cd,{de,ef}}}.txt`, [], { cwd: tmpDir }), {
        ...
        it(`should support glob patterns with regexp character classes`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo +([a-d]).txt`, [], { cwd: tmpDir }), {
              stdout: `abcd.txt\n`,
            });

            await expectResult(bufferResult(`echo +([1-5]).txt`, [], { cwd: tmpDir }), {
              stdout: `12345.txt\n`,
            });

            await expectResult(bufferResult(`echo +([a-d])+([x-z])+([0-9]).txt`, [], { cwd: tmpDir }), {
        ...
        it(`should support glob patterns with regexp groups`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            ...
            await expectResult(bufferResult(`echo (ab|xy)(12|89).txt`, [], { cwd: tmpDir }), {
              stdout: `ab12.txt xy89.txt\n`,
            });

            await expectResult(bufferResult(`echo foo(x!(a)|y!(b))1.txt`, [], { cwd: tmpDir }), {
    ...
    describe(`Functionality`, () => {
      describe(`Errors`, () => {
        it(`should throw recoverable errors when no matches are found`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            await expectResult(bufferResult(`echo * && echo OK || echo KO`, [], { cwd: tmpDir }), {
              exitCode: 0,
              stdout: `KO\n`,
        ...
        it(`should include a brace expansion notice when no matches are found for a brace expansion pattern`, async () => {
          await xfs.mktempPromise(async (tmpDir) => {
            await expectResult(bufferResult(`echo {foo,bar}`, [], { cwd: tmpDir }), {
              exitCode: 1,
              stdout: ``,
    ...
    describe(`Integrations`, () => {
      it(`should work with environment variables`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo $DIRNAME/*`, [], { cwd: tmpDir, env: { DIRNAME: `subdir` } }), {
      ...
      it(`should work with subshells`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo $(echo subdir)/*`, [], { cwd: tmpDir, env: { DIRNAME: `subdir` } }), {
            stdout: `subdir/a.txt subdir/b.txt subdir/c.txt\n`,
          });
        });
      });
      ...
      it(`should work with arithmetics`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`echo $(( 1000 + 234 ))/*`, [], { cwd: tmpDir }), {
  ...
  describe(`Calculations`, () => {
    it(`should support integers`, async () => {
      await expectResult(bufferResult(`echo $(( 1 ))`), {
        exitCode: 0,
        stdout: `1\n`,
      });

      await expectResult(bufferResult(`echo $(( 134 ))`), {
        exitCode: 0,
        stdout: `134\n`,
      });

      await expectResult(bufferResult(`echo $(( 5693 ))`), {
        exitCode: 0,
        stdout: `5693\n`,
      });

      await expect(bufferResult(`echo $(( 5.93 ))`)).rejects.toThrowError(
        /Invalid number: "5\.93", only integers are allowed/,
      );
    });

    it(`should support operations`, async () => {
      await expectResult(bufferResult(`echo $(( 1 + 2 -4 ))`), {
        exitCode: 0,
        stdout: `-1\n`,
      });

      await expectResult(bufferResult(`echo $(( 134 / 3 ))`), {
        exitCode: 0,
        stdout: `44\n`,
      });
      ...
      await expectResult(bufferResult(`echo $(( 4 * (2 + 3) * 5 ))`), {
        exitCode: 0,
        stdout: `100\n`,
      });
    ...
    it(`should have left associativity`, async () => {
      await expectResult(bufferResult(`echo $(( 7 - 2 - 3 ))`), {
        exitCode: 0,
        stdout: `2\n`,
      });

      await expectResult(bufferResult(`echo $(( 32 / 4 / 8 ))`), {
        exitCode: 0,
        stdout: `1\n`,
      });

      await expectResult(bufferResult(`echo $(( 32 + 64 * 2 / 4 / 8 - 9 ))`), {
        exitCode: 0,
        stdout: `27\n`,
      });
    });
    ...
    it(`should support arguments`, async () => {
      await expectResult(bufferResult(`echo $(( $0 + 2 ))`, [`3`]), {
        exitCode: 0,
        stdout: `5\n`,
      });

      await expectResult(bufferResult(`echo $(( $0 / $1 ))`, [`9`, `3`]), {
        exitCode: 0,
        stdout: `3\n`,
      });
    });
    ...
    it(`should support variables and env`, async () => {
      ...
      await expectResult(bufferResult(`echo $(( $three + 2 ))`, [], opts), {
        exitCode: 0,
        stdout: `5\n`,
      });

      await expectResult(bufferResult(`echo $(( $four * $two ))`, [], opts), {
        exitCode: 0,
        stdout: `8\n`,
      });

      await expectResult(bufferResult(`echo $(( three + one ))`, [], opts), {
        exitCode: 0,
        stdout: `4\n`,
      });

      await expectResult(bufferResult(`echo $((isThisDeepEnough+ one))`, [], opts), {
        exitCode: 0,
        stdout: `5\n`,
      });
    });
  ...
  describe(`Background jobs`, () => {
    ...
    it(`should print errors to stderr and always exit with exit code 0`, async () => {
      const { stdout, stderr, exitCode } = await bufferResult(`echo $THIS_VARIABLE_DOES_NOT_EXIST &`, [], {
        tty: true,
      });
    ...
    it(`should wait for all background jobs to finish before resolving`, async () => {
      await expect(
        Promise.race([bufferResult(`sleep 0.4 && echo foo`, [], { tty: false }), setTimeout(300)]),
      ).resolves.toBeUndefined();

      await expectResult(Promise.race([bufferResult(`sleep 0.4 && echo foo`, [], { tty: false }), setTimeout(500)]), {
        stdout: `foo\n`,
        stderr: ``,
        exitCode: 0,
      });
  ...
  describe(`Builtins`, () => {
    describe(`cd`, () => {
      it(`should throw recoverable errors when the target is not a directory`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          ...
          await expectResult(bufferResult(`cd file && echo OK || echo KO`, [], { cwd: tmpDir }), {
            exitCode: 0,
            stdout: `KO\n`,
      ...
      it(`should throw recoverable errors when the target does not exist`, async () => {
        await xfs.mktempPromise(async (tmpDir) => {
          await expectResult(bufferResult(`cd doesnt-exist && echo OK || echo KO`, [], { cwd: tmpDir }), {
            exitCode: 0,
            stdout: `KO\n`,
    ...
    describe(`sleep`, () => {
      it(`should throw recoverable errors when the operand is missing`, async () => {
        await expectResult(bufferResult(`sleep && echo OK || echo KO`), {
          exitCode: 0,
          stdout: `KO\n`,
      ...
      it(`should throw recoverable errors when the operand is an invalid time interval`, async () => {
        await expectResult(bufferResult(`sleep invalid && echo OK || echo KO`), {
          exitCode: 0,
          stdout: `KO\n`,
