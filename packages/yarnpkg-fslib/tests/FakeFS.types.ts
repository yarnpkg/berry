...
Promise.resolve().then(async () => {
  ...
  assertEqual<Array<Filename | PortablePath>>()(
    await fakeFs.readdirPromise(PortablePath.dot, { recursive: Boolean() }),
    true,
  );

  assertEqual<Array<PortablePath>>()(await fakeFs.readdirPromise(PortablePath.dot, { recursive: true }), true);
  assertEqual<Array<Dirent<PortablePath>>>()(
    await fakeFs.readdirPromise(PortablePath.dot, { recursive: true, withFileTypes: true }),
    true,
  );

  assertEqual<Array<Filename>>()(await fakeFs.readdirPromise(PortablePath.dot, { recursive: false }), true);
  assertEqual<Array<DirentNoPath>>()(
    await fakeFs.readdirPromise(PortablePath.dot, { recursive: false, withFileTypes: true }),
    true,
  );
});
