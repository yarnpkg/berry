...
export class MountFS<MountedFS extends MountableFS> extends BasePortableFakeFS {
  ...
  private readonly mountInstances: Map<
    string,
    {
      childFs: MountedFS;
      expiresAt: number;
  ...
  constructor({
    ...
    useCache = true,
    maxAge = 5000,
    typeCheck = constants.S_IFREG,
    getMountPoint,
    factoryPromise,
    factorySync,
  }: MountFSOptions<MountedFS>) {
  ...
  readSync(fd: number, buffer: Buffer, offset: number, length: number, position: number) {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`readSync`);
  ...
  async writePromise(
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`write`);
  ...
  writeSync(fd: number, buffer: Buffer | string, offset?: number, length?: number, position?: number): number {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`writeSync`);
  ...
  async closePromise(fd: number) {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`close`);
  ...
  closeSync(fd: number) {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`closeSync`);
  ...
  async fstatPromise(fd: number, opts?: { bigint: boolean }) {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`fstat`);
  ...
  async fchmodPromise(fd: number, mask: number): Promise<void> {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`fchmod`);
  ...
  fchmodSync(fd: number, mask: number): void {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`fchmodSync`);
  ...
  async fchownPromise(fd: number, uid: number, gid: number): Promise<void> {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`fchown`);
  ...
  fchownSync(fd: number, uid: number, gid: number): void {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`fchownSync`);
  ...
  async ftruncatePromise(fd: number, len?: number): Promise<void> {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`ftruncate`);
  ...
  ftruncateSync(fd: number, len?: number): void {
    ...
    const entry = this.fdMap.get(fd);
    if (typeof entry === `undefined`) throw errors.EBADF(`ftruncateSync`);
  ...
  private async makeCallPromise<T>(
    ...
    discard: () => Promise<T>,
    accept: (mountFS: MountedFS, mountInfo: { archivePath: PortablePath; subPath: PortablePath }) => Promise<T>,
  ...
  private makeCallSync<T>(
    p: FSPath<PortablePath>,
    discard: () => T,
    accept: (mountFS: MountedFS, mountInfo: { archivePath: PortablePath; subPath: PortablePath }) => T,
    { requireSubpath = true }: { requireSubpath?: boolean } = {},
  ): T {
