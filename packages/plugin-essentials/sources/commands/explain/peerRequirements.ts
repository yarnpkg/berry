...
export default class ExplainPeerRequirementsCommand extends BaseCommand {
  static paths = [[`explain`, `peer-requirements`]];
  ...
  static usage = Command.Usage({
    ...
    `,
    examples: [
      [`Explain the corresponding set of peer requirements for a hash`, `$0 explain peer-requirements p1a4ed`],
      [`List all sets of peer requirements`, `$0 explain peer-requirements`],
    ],
  });

  hash = Option.String({
    validator: t.cascade(t.isString(), [t.matchesRegExp(/^p[0-9a-f]{5}$/)]),
  });
...
export async function explainPeerRequirements(
  peerRequirementsHash: string,
  project: Project,
  opts: { stdout: Writable },
) {
        ...
        case PeerWarningType.NotCompatibleAggregate:
          {
            report.reportInfo(
              MessageName.UNNAMED,
              `We have a problem with ${formatUtils.pretty(project.configuration, warning.requested, formatUtils.Type.IDENT)}, which is provided with version ${structUtils.prettyReference(project.configuration, warning.version)}.`,
            );
            report.reportInfo(
              MessageName.UNNAMED,
            ...
            for (const dependent of warning.requesters.values()) {
              ...
              if (!dependentPkg) throw new Error(`Assertion failed: Expected the package to be registered`);

              const descriptor = dependentPkg?.peerDependencies.get(warning.requested.identHash);
              if (!descriptor) throw new Error(`Assertion failed: Expected the package to list the peer dependency`);
              ...
              report.reportInfo(
                null,
                `  ${mark} ${structUtils.prettyLocator(project.configuration, dependent)} (via ${structUtils.prettyRange(project.configuration, descriptor.range)})`,
              );
            }
            ...
            if (transitiveLinks.length > 0) {
              ...
              report.reportInfo(
                MessageName.UNNAMED,
                `However, those packages themselves have more dependencies listing ${structUtils.prettyIdent(project.configuration, warning.requested)} as peer dependency:`,
              );
              report.reportSeparator();

              for (const link of transitiveLinks) {
                const linkPkg = project.storedPackages.get(link.locatorHash);
                if (!linkPkg) throw new Error(`Assertion failed: Expected the package to be registered`);
                ...
                report.reportInfo(
                  null,
                  `  ${mark} ${structUtils.prettyLocator(project.configuration, link)} (via ${structUtils.prettyRange(project.configuration, descriptor.range)})`,
                );
              }
            }
            ...
            if (allRanges.length > 1) {
              ...
              if (resolvedRange === null) {
                ...
                report.reportInfo(
                  MessageName.UNNAMED,
                  `Your best option may be to try to upgrade some dependencies with ${formatUtils.pretty(project.configuration, `yarn up`, formatUtils.Type.CODE)}, or silence the warning via ${formatUtils.pretty(project.configuration, `logFilters`, formatUtils.Type.CODE)}.`,
                );
              } else {
                report.reportInfo(
                  MessageName.UNNAMED,
                  `Put together, the final range we computed is ${formatUtils.pretty(project.configuration, resolvedRange, formatUtils.Type.RANGE)}`,
                );
              }
            }
          }
        ...
        default:
          {
            report.reportInfo(
              MessageName.UNNAMED,
              `The ${formatUtils.pretty(project.configuration, `yarn explain peer-requirements`, formatUtils.Type.CODE)} command doesn't support this warning type yet.`,
            );
          }
          break;
      }
