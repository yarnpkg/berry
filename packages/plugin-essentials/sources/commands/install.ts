...
export default class YarnCommand extends BaseCommand {
  ...
  async execute() {
    ...
    if (deprecationExitCode !== null) return deprecationExitCode;

    const updateMode = this.mode === InstallMode.UpdateLockfile;
    if (updateMode && (this.immutable || this.immutableCache))
      throw new UsageError(
        `${formatUtils.pretty(configuration, `--immutable`, formatUtils.Type.CODE)} and ${formatUtils.pretty(configuration, `--immutable-cache`, formatUtils.Type.CODE)} cannot be used with ${formatUtils.pretty(configuration, `--mode=update-lockfile`, formatUtils.Type.CODE)}`,
      );
      ...
      const fixReport = await StreamReport.start(
        ...
        async (report) => {
          ...
          if (await autofixLegacyPlugins(configuration, immutable)) {
            report.reportInfo(
              MessageName.AUTOMERGE_SUCCESS,
              `Automatically removed core plugins that are now builtins 👍`,
            );
            changed = true;
          }
      ...
      const telemetryReport = await StreamReport.start(
        ...
        async (report) => {
          if (Configuration.telemetry?.isNew) {
            ...
            report.reportInfo(
              MessageName.TELEMETRY_NOTICE,
              `Yarn will periodically gather anonymous telemetry: https://yarnpkg.com/advanced/telemetry`,
            );
            report.reportInfo(
              MessageName.TELEMETRY_NOTICE,
              `Run ${formatUtils.pretty(configuration, `yarn config set --home enableTelemetry 0`, formatUtils.Type.CODE)} to disable`,
            );
            report.reportSeparator();
          } else if (Configuration.telemetry?.shouldShowTips) {
            const data = (await httpUtils
              .get(`https://repo.yarnpkg.com/tags`, { configuration, jsonResponse: true })
              .catch(() => null)) as {
            ...
            if (data !== null) {
              ...
              if (newVersion) {
                ...
                report.reportInfo(
                  MessageName.VERSION_NOTICE,
                  `${formatUtils.applyStyle(configuration, `A new ${newVersion[0]} version of Yarn is available:`, formatUtils.Style.BOLD)} ${structUtils.prettyReference(configuration, newVersion[1])}!`,
                );
                report.reportInfo(
                  MessageName.VERSION_NOTICE,
                  `Upgrade now by running ${formatUtils.pretty(configuration, `yarn set version ${newVersion[1]}`, formatUtils.Type.CODE)}`,
                );
                report.reportSeparator();
              } else {
                ...
                if (tip) {
                  report.reportInfo(
                    ...
                    formatUtils.pretty(configuration, tip.message, formatUtils.Type.MARKDOWN_INLINE),
                  );

                  if (tip.url) report.reportInfo(MessageName.TIPS_NOTICE, `Learn more at ${tip.url}`);
    ...
    if (enableHardenedMode && typeof configuration.sources.get(`enableHardenedMode`) === `undefined`) {
      await StreamReport.start(
        ...
        async (report) => {
          ...
          report.reportWarning(
            MessageName.UNNAMED,
            `It will prevent malicious lockfile manipulations, in exchange for a slower install time. You can opt-out if necessary; check our ${formatUtils.applyHyperlink(configuration, `documentation`, `https://yarnpkg.com/features/security#hardened-mode`)} for more details.`,
          );
          report.reportSeparator();
        },
      );
    ...
    if (this.refreshLockfile ?? enableHardenedMode) project.lockfileNeedsRefresh = true;
...
async function autofixMergeConflicts(configuration: Configuration, immutable: boolean) {
  ...
  if (!(await xfs.existsPromise(lockfilePath))) return false;
  ...
  if (immutable)
    throw new ReportError(
      MessageName.AUTOMERGE_IMMUTABLE,
  ...
  if (commits.code !== 0)
    throw new ReportError(
      ...
      `Git returned an error when trying to find the commits pertaining to the conflict`,
    );
    ...
    commits.stdout
      ...
      .map(async (hash) => {
        ...
        if (content.code !== 0)
          throw new ReportError(
            MessageName.AUTOMERGE_GIT_ERROR,
            `Git returned an error when trying to access the lockfile content in ${hash}`,
          );
        ...
        try {
          ...
          throw new ReportError(
            MessageName.AUTOMERGE_FAILED_TO_PARSE,
            `A variant of the conflicting lockfile failed to parse`,
          );
        }
      }),
  );
  ...
  for (const variant of variants) {
    ...
    for (const key of Object.keys(variant)) {
      ...
      if (typeof checksum === `string` && checksum.includes(`/`)) continue;
...
async function autofixLegacyPlugins(configuration: Configuration, immutable: boolean) {
  ...
  const changed = await Configuration.updateConfiguration(
    configuration.projectCwd,
    {
      plugins: (plugins) => {
        ...
        const filteredPlugins = plugins.filter((plugin: { spec: string; path: PortablePath }) => {
          ...
          if (isLegacy) legacyPlugins.push(resolvedPath);

          return !isLegacy;
        });

        if (filteredPlugins.length === 0) return Configuration.deleteProperty;

        if (filteredPlugins.length === plugins.length) return plugins;
  ...
  await Promise.all(
    legacyPlugins.map(async (pluginPath) => {
