    ...
    const report = await StreamReport.start(
      ...
      async (report) => {
        ...
        if (this.name.match(/^\.{0,2}[\\/]/) || npath.isAbsolute(this.name)) {
          ...
          report.reportInfo(
            MessageName.UNNAMED,
            `Reading ${formatUtils.pretty(configuration, candidatePath, formatUtils.Type.PATH)}`,
          );
          ...
          if (this.name.match(/^https?:/)) {
            ...
            if (locator.reference !== `unknown` && !semver.valid(locator.reference))
              throw new ReportError(
                MessageName.UNNAMED,
                `Official plugins only accept strict version references. Use an explicit URL if you wish to download them from another location.`,
              );
          ...
          report.reportInfo(
            MessageName.UNNAMED,
            `Downloading ${formatUtils.pretty(configuration, pluginUrl, `green`)}`,
          );
...
export async function savePlugin(
  pluginSpec: string,
  pluginBuffer: Buffer,
  { checksum = true, project, report }: { checksum?: boolean; project: Project; report: Report },
) {
  ...
  report.reportInfo(
    MessageName.UNNAMED,
    `Saving the new plugin in ${formatUtils.pretty(configuration, relativePath, `magenta`)}`,
  );
