      ...
      async (report) => {
        ...
        if (!configuration.plugins.has(pluginName))
          throw new UsageError(
            `${structUtils.prettyIdent(configuration, pluginIdent)} isn't referenced by the current configuration`,
          );
        ...
        if (xfs.existsSync(absolutePath)) {
          report.reportInfo(
            MessageName.UNNAMED,
            `Removing ${formatUtils.pretty(configuration, relativePath, formatUtils.Type.PATH)}...`,
          );
