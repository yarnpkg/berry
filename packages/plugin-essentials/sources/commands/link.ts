...
export default class LinkCommand extends BaseCommand {
  ...
  static usage: Usage = Command.Usage({
    ...
    `,
    examples: [
      [`Register one or more remote workspaces for use in the current project`, `$0 link ~/ts-loader ~/jest`],
  ...
  async execute() {
      ...
      if (this.all) {
        ...
        if (!workspace2.manifest.name)
          throw new UsageError(
            `The target workspace at '${destination}' doesn't have a name and thus cannot be linked`,
          );

        if (workspace2.manifest.private && !this.private)
          throw new UsageError(
            `The target workspace at '${destination}' is marked private - use the --private flag to link it anyway`,
          );
    ...
    for (const workspace of linkedWorkspaces) {
      ...
      const target = this.relative ? ppath.relative(project.cwd, workspace.cwd) : workspace.cwd;
