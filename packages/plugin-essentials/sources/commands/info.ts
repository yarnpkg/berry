...
export default class InfoCommand extends BaseCommand {
  ...
  async execute() {
    ...
    if (!workspace && !this.all) throw new WorkspaceRequiredError(project.cwd, this.context.cwd);
    ...
    const traverseWorkspace = (workspace: Workspace, { recursive }: { recursive: boolean }) => {
      ...
      while (pass.length > 0) {
        ...
        if (structUtils.isVirtualLocator(pkg)) pass.push(structUtils.devirtualizeLocator(pkg).locatorHash);
    ...
    const traverseAllWorkspaces = ({ recursive }: { recursive: boolean }) => {
      ...
      for (const workspace of project.workspaces)
        for (const pkg of traverseWorkspace(workspace, { recursive })) aggregate.set(pkg.locatorHash, pkg);
    ...
    const findSelectedSet = ({ all, recursive }: { all: boolean; recursive: boolean }) => {
      ...
      const matchers = this.patterns.map((pattern) => {
        ...
        const uvPatternLocator = patternIsVirtual ? structUtils.devirtualizeLocator(patternLocator) : patternLocator;
        ...
        return (pkg: Package) => {
          ...
          const uvPkgLocator = pkgIsVirtual ? structUtils.devirtualizeLocator(pkg) : pkg;

          // If the pattern is explicitly virtual, we only accept this one virtual package, never the others
          if (patternIsVirtual && pkgIsVirtual && patternLocator.reference !== pkg.reference) return false;
    ...
    if (selection.length === 0) throw new UsageError(`No package matched your request`);
    ...
    const builtinInfoBuilders: Array<Exclude<Hooks["fetchPackageInfo"], undefined>> = [
      ...
      async (pkg, extra, registerData) => {
        ...
        const size = typeof stat !== `undefined` ? ([stat.size, formatUtils.Type.SIZE] as const) : undefined;
    ...
    for (const pkg of selection) {
      ...
      const registerData = (
        namespace: string,
        info: Array<formatUtils.Tuple> | { [key: string]: formatUtils.Tuple | undefined },
      ) => {
      ...
      if (pkg.dependencies.size > 0 && !isVirtual) {
        registerData(
          ...
          [...pkg.dependencies.values()].map((dependency) => {
            ...
            const resolution =
              typeof resolutionHash !== `undefined` ? project.storedPackages.get(resolutionHash) ?? null : null;
      ...
      if (pkg.peerDependencies.size > 0 && isVirtual) {
        registerData(
          ...
          [...pkg.peerDependencies.values()].map((peerDependency) => {
            ...
            const resolution = resolutionHash !== null ? project.storedPackages.get(resolutionHash) ?? null : null;
