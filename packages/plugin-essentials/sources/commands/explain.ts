...
export async function getErrorCodeDetails(configuration: Configuration) {
  const version = miscUtils.isTaggedYarnVersion(YarnVersion) ? YarnVersion : await resolveTag(configuration, `canary`);
  ...
  return new Map<string, string>(
    Array.from(raw.toString().matchAll(ERROR_CODE_DOC_REGEXP), ({ groups }) => {
      if (!groups) throw new Error(`Assertion failed: Expected the match to have been successful`);

      const expectedName = getCodeName(groups.code);
      if (groups.name !== expectedName)
        throw new Error(
          `Assertion failed: Invalid error code data: Expected "${groups.name}" to be named "${expectedName}"`,
        );
...
export default class ExplainCommand extends BaseCommand {
  ...
  static usage = Command.Usage({
    ...
    `,
    examples: [
      [`Explain an error code`, `$0 explain YN0006`],
      [`List all error codes`, `$0 explain`],
    ],
  });

  code = Option.String({
    required: false,
    validator: t.applyCascade(t.isString(), [t.matchesRegExp(/^YN[0-9]{4}$/)]),
  });
    ...
    if (typeof this.code !== `undefined`) {
      ...
      const content =
        typeof details !== `undefined`
          ? formatUtils.jsonOrPretty(
              this.json,
              configuration,
              formatUtils.tuple(formatUtils.Type.MARKDOWN, {
                text: details,
      ...
      const tree: treeUtils.TreeNode = {
        children: miscUtils.mapAndFilter(Object.entries(MessageName), ([key, value]) => {
          if (Number.isNaN(Number(key))) return miscUtils.mapAndFilter.skip;
