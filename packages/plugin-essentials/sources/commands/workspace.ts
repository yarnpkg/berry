...
export default class WorkspaceCommand extends BaseCommand {
  ...
  async execute() {
    ...
    if (!cwdWorkspace) throw new WorkspaceRequiredError(project.cwd, this.context.cwd);

    const candidates = project.workspaces;
    const candidatesByName = new Map(
      candidates.map((workspace) => [structUtils.stringifyIdent(workspace.anchoredLocator), workspace]),
    );
