...
function whySimple(
  project: Project,
  identHash: IdentHash,
  { configuration, peers }: { configuration: Configuration; peers: boolean },
) {
...
function whyRecursive(
  ...
  for (const workspace of sortedWorkspaces) markAllDependents(workspace.anchoredPackage);
  ...
  for (const workspace of sortedWorkspaces) printAllDependents(workspace.anchoredPackage, rootChildren, null);
