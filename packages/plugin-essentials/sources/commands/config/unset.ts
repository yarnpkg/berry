...
export default class ConfigUnsetCommand extends BaseCommand {
  ...
  static usage: Usage = Command.Usage({
    ...
    `,
    examples: [
      [`Unset a simple configuration setting`, `yarn config unset initScope`],
      [`Unset a complex configuration setting`, `yarn config unset packageExtensions`],
      [`Unset a nested configuration setting`, `yarn config unset npmScopes.company.npmRegistryServer`],
    ],
  });
  ...
  async execute() {
    ...
    const assertProjectCwd = () => {
      if (!configuration.projectCwd) throw new UsageError(`This command must be run from within a project folder`);
    ...
    const updateConfiguration: (patch: (current: any) => any) => Promise<boolean> = this.home
        ...
        await updateConfiguration((current) => {
          if (!hasPath(current, this.name)) {
            report.reportWarning(
              MessageName.UNNAMED,
              `Configuration doesn't contain setting ${this.name}; there is nothing to unset`,
            );
            bailedOutEarly = true;
            return current;
          }

          const clone = path ? cloneDeep(current) : { ...current };
