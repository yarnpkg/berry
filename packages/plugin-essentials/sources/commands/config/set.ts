...
export default class ConfigSetCommand extends BaseCommand {
  ...
  static usage: Usage = Command.Usage({
    ...
    `,
    examples: [
      [`Set a simple configuration setting (a string, a number, or a boolean)`, `yarn config set initScope myScope`],
      [
  ...
  async execute() {
    ...
    const report = await StreamReport.start(
      ...
      async (report) => {
        ...
        report.reportInfo(
          MessageName.UNNAMED,
          `Successfully set ${this.name} to ${inspect(requestedObject, {
            depth: Infinity,
