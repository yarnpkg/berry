...
export default class ConfigCommand extends BaseCommand {
  ...
  async execute() {
    ...
    const names = this.names.length > 0 ? [...new Set(this.names)].sort() : [...configuration.settings.keys()].sort();
      ...
      async (report) => {
        if (configuration.invalid.size > 0 && !this.json) {
          for (const [key, source] of configuration.invalid)
            report.reportError(
              ...
              `Invalid configuration key "${key}" in ${source}`,
            );
        ...
        if (this.json) {
          for (const name of names) {
            ...
            const sourceAsNativePath = source && source[0] !== `<` ? npath.fromPortablePath(source) : source;
          ...
          for (const name of names) {
            if (this.noDefaults && !configuration.sources.has(name)) continue;
            ...
            const fields: treeUtils.TreeMap = {
              Description: {
                label: `Description`,
                value: formatUtils.tuple(formatUtils.Type.MARKDOWN, {
          ...
          if (names.length !== 1) trailingValue = undefined;
