    ...
    const report = await StreamReport.start({
      ...
      if (this.json) {
        ...
        for (const key of keys) {
          ...
          const effective = configuration.getSecret(key);
        ...
        if (this.why || this.verbose) {
          ...
          for (const [key, description] of keysAndDescriptions) {
            report.reportInfo(null, `${key.padEnd(maxKeyLength, ` `)}   ${description.padEnd(maxDescriptionLength, ` `)}   ${inspect(configuration.getSecret(key), inspectConfig)}`);
          }
        } else {
          for (const key of keys) {
            report.reportInfo(null, `${key.padEnd(maxKeyLength, ` `)}   ${inspect(configuration.getSecret(key), inspectConfig)}`);
          }
        }
      }
    });
