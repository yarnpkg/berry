...
export function applyModifier(descriptor: Descriptor, modifier: Modifier) {
  ...
  if (semver.valid(selector)) selector = `${modifier}${descriptor.range}`;
...
export async function extractDescriptorFromPath(
  path: string,
  {cache, cwd, workspace}: {cache: Cache; cwd: string; workspace: Workspace},
) {
  if (!posix.isAbsolute(path)) path = posix.resolve(cwd, path);

  const project = workspace.project;

  const descriptor = await fetchDescriptorFrom(structUtils.makeIdent(null, `archive`), path, {
...
export async function getSuggestedDescriptors(
  request: Descriptor,
      ...
      case Strategy.REUSE:
        {
          for (const {descriptor, locators} of (await findProjectDescriptors(request, {project, target})).values()) {
            let reason = `Reuse ${structUtils.prettyDescriptor(
              project.configuration,
              descriptor,
            )} (originally used by ${structUtils.prettyLocator(project.configuration, locators[0])}`;
      ...
      case Strategy.CACHE:
        {
          for (const descriptor of project.storedDescriptors.values()) {
            if (descriptor.identHash === request.identHash) {
              const reason = `Reuse ${structUtils.prettyDescriptor(
                project.configuration,
                descriptor,
      ...
      case Strategy.PROJECT:
        {
          for (const workspace of project.workspacesByIdent.get(request.identHash) || []) {
            const reason = `Attach ${structUtils.prettyWorkspace(
              project.configuration,
              workspace,
            )} (local workspace at ${workspace.cwd})`;
      ...
      case Strategy.LATEST:
        {
          if (request.range !== `unknown`) {
            const reason = `Use ${structUtils.prettyRange(
              project.configuration,
              request.range,
            ...
            const reason = `Resolve from latest ${project.configuration.format(
            ...
            if (latest) {
              ...
              const reason = `Use ${structUtils.prettyDescriptor(
                project.configuration,
                latest,
...
export async function fetchDescriptorFrom(
  ident: Ident,
  range: string,
  {project, cache}: {project: Project; cache: Cache},
) {
