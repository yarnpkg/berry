...
export function makeWorkspaceDescriptor(workspace: Workspace, modifier: Modifier) {
  return structUtils.makeDescriptor(
    ...
    `${WORKSPACE_PROTOCOL}${toWorkspaceModifier(modifier)}`,
  );
}
...
export async function extractDescriptorFromPath(
  path: PortablePath,
  { cwd, workspace }: { cwd: PortablePath; workspace: Workspace },
) {
...
export async function getSuggestedDescriptors(
  ...
  const [requestRange, requestTag] =
    request.range !== `unknown`
  ...
  const existing =
    typeof workspace !== `undefined` && workspace !== null
  ...
  for (const strategy of strategies) {
    ...
    switch (strategy) {
      ...
      case Strategy.REUSE:
        {
          await trySuggest(async () => {
            for (const { descriptor, locators } of (
              await findProjectDescriptors(request, { project, target })
            ).values()) {
              ...
              if (locators.length === 1 && locators[0].locatorHash === workspace.anchoredLocator.locatorHash)
                if (strategies.includes(Strategy.KEEP)) continue;

              let reason = `(originally used by ${structUtils.prettyLocator(project.configuration, locators[0])}`;

              reason +=
                locators.length > 1 ? ` and ${locators.length - 1} other${locators.length > 2 ? `s` : ``})` : `)`;
...
export async function fetchDescriptorFrom(
  ident: Ident,
  range: string,
  { project, cache, workspace, preserveModifier = true, modifier }: FetchDescriptorFromOptions,
) {
  ...
  let { protocol, source, params, selector } = structUtils.parseRange(
