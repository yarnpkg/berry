...
describe(`TAG_REGEXP`, () => {
  const validTags = [`canary`, `latest`, `next`, `legacy_v1`];
  ...
  it(`should hide secrets`, async () => {
    await initializeConfiguration(
      ...
      async (dir) => {
        ...
        const secondToken = configuration
          .getSpecial(`npmScopes`, {
            hideSecrets: true,
          })
          .get(`myScope`)
          .get(`npmAuthToken`);
    ...
    it(`should merge mergeable array properties`, async () => {
      await initializeConfiguration(
        ...
        async (dir) => {
          ...
          expect(configuration.get(`unsafeHttpWhitelist`)).toEqual([`example.com`]);
          ...
          configuration.useWithSource(
            `override file`,
            {
              ...
              unsafeHttpWhitelist: [`yarnpkg.com`],
            },
            dir,
            { overwrite: true },
          );
          ...
          expect(configuration.get(`unsafeHttpWhitelist`)).toEqual([`yarnpkg.com`]);
        },
      );
    });
  });
    ...
    await initializeConfiguration(
      {
        ...
        map: {
          foo: {
            // empty objects are discarded by `stringifySyml`.
      ...
      await initializeConfiguration(
        ...
        async (dir) => {
          ...
          await xfs.writeFilePromise(
            `${workspaceDirectory}/.yarnrc.yml` as PortablePath,
            stringifySyml({
              string: `bar`,
              stringReset: {
                onConflict: `reset`,
          ...
          await xfs.writeFilePromise(
            `${workspaceDirectory2}/.yarnrc.yml` as PortablePath,
            stringifySyml({
              stringHardReset: `baz`,
          ...
          expect(configuration.get(`shape`)).toEqual(
            new Map<string, any>([
              [`number`, 2],
              [`string`, `foo`],
            ]),
          );
          expect(configuration.get(`shapeReset`)).toEqual(
            new Map<string, any>([
              [`number`, 2],
              [`string`, `default`],
            ]),
          );
          expect(configuration.get(`shapeExtend`)).toEqual(
            new Map<string, any>([
              [`number`, 2],
              [`string`, `foo`],
            ]),
          );
          expect(configuration.get(`shapeHardReset`)).toEqual(
            new Map<string, any>([
              [`number`, 0],
              [`string`, `baz`],
            ]),
          ...
          expect(configuration.get(`mapExtend`)).toEqual(
            new Map([
              [
                `foo`,
                new Map<string, any>([
          ...
          expect(configuration.get(`mapHardReset`)).toEqual(
            new Map([
              [
                `baz`,
                new Map<string, any>([
          ...
          expect(configuration.get(`anyExtend`)).toEqual({
            bar: { number: `2`, string: `bar` },
            foo: { string: `foo` },
          });
