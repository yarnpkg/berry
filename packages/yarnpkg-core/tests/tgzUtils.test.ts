...
describe(`tgzUtils`, () => {
  describe(`convertToZip`, () => {
    it(`should be able to convert a tar with a lot of files without getting max callstack`, async () => {
      ...
      await expect(tgzUtils.convertToZip(data, {compressionLevel: 0, poolSize: 2})).resolves.toBeTruthy();
    });
    ...
    it(`should be able to convert a tgz without any workers`, async () => {
      ...
      await expect(tgzUtils.convertToZip(data, {compressionLevel: 0, poolSize: 0})).resolves.toBeTruthy();
    });
  });
});
