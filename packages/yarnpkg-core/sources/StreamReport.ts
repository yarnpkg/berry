...
const supportsEmojis =
  [`iTerm.app`, `Apple_Terminal`, `WarpTerminal`, `vscode`].includes(process.env.TERM_PROGRAM!) ||
...
const defaultStyle =
  (supportsEmojis &&
    Object.keys(PROGRESS_STYLES).find((name) => {
...
export function formatName(
  name: MessageName | null,
  { configuration, json }: { configuration: Configuration; json: boolean },
) {
  if (!configuration.get(`enableMessageNames`)) return ``;
...
export function formatNameWithHyperlink(
  name: MessageName | null,
  { configuration, json }: { configuration: Configuration; json: boolean },
) {
  ...
  if (name === null || name === MessageName.UNNAMED) return code;
 ...
 * @internal
 */
export async function reportOptionDeprecations(
  { configuration, stdout, forceError }: { configuration: Configuration; stdout: Writable; forceError?: boolean },
  checks: Array<{ option: unknown; message: string; error?: boolean; callback?: () => void }>,
) {
...
export class StreamReport extends Report {
  static async start(opts: StreamReportOptions, cb: (report: StreamReport) => Promise<void>) {
    ...
    if (opts.includeVersion)
      report.reportInfo(
        MessageName.UNNAMED,
        formatUtils.applyStyle(opts.configuration, `Yarn ${YarnVersion}`, formatUtils.Style.BOLD),
      );
  ...
  constructor({
    ...
    // Setup progress
    if (
      configuration.get(`enableProgressBars`) &&
      !json &&
      (stdout as WriteStream).isTTY &&
      (stdout as WriteStream).columns > 22
    ) {
      const styleName = configuration.get(`progressBarStyle`) || defaultStyle;
      if (!Object.hasOwn(PROGRESS_STYLES, styleName)) throw new Error(`Assertion failed: Invalid progress bar style`);
  ...
  getRecommendedLength() {
    ...
    const recommendedLength =
      this.progressStyle !== null ? (this.stdout as WriteStream).columns - 1 : super.getRecommendedLength();
    ...
    return {
      ...
      reportFooter: (elapsedTime) => {
        ...
        if (this.configuration.get(`enableTimers`) && elapsedTime > 200)
          this.reportInfo(
            null,
            `└ Completed in ${formatUtils.pretty(this.configuration, elapsedTime, formatUtils.Type.DURATION)}`,
          );
        else this.reportInfo(null, `└ Completed`);
  ...
  reportProgress(progressIt: ProgressIterable) {
    if (this.progressStyle === null) return { ...Promise.resolve(), stop: () => {} };
  ...
  private writeLines(lines: Array<string>, { truncate }: { truncate?: boolean } = {}) {
    ...
    for (const line of lines) this.stdout.write(`${this.truncate(line, { truncate })}\n`);
  ...
  private clearProgress({ delta = 0, clear = false }: { delta?: number; clear?: boolean }) {
    if (this.progressStyle === null) return;
  ...
  private writeProgress() {
    ...
    for (const progress of this.progress.values()) {
      ...
      this.stdout.write(
        `${formatUtils.pretty(this.configuration, `➤`, `blueBright`)} ${prefix}${spinner}${progressBar}${title}\n`,
      );
    }
  ...
  private truncate(str: string, { truncate }: { truncate?: boolean } = {}) {
    if (this.progressStyle === null) truncate = false;

    if (typeof truncate === `undefined`) truncate = this.configuration.get(`preferTruncatedLines`);

    // The -1 is to account for terminals that would wrap after
    // the last column rather before the first overwrite
    if (truncate) str = sliceAnsi(str, 0, (this.stdout as WriteStream).columns - 1);

    return str;
  }

  private formatName(name: MessageName | null) {
    if (!this.includeNames) return ``;
  ...
  private formatIndent() {
    return this.level > 0 || !this.forceSectionAlignment ? `│ `.repeat(this.indent) : `${SINGLE_LINE_CHAR} `;
  }
}
