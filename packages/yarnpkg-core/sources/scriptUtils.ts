...
export async function makeScriptEnv({
  project,
  locator,
  binFolder,
  ignoreCorepack,
  lifecycleScript,
  baseEnv = project?.configuration.env ?? process.env,
}: {
  project?: Project;
  locator?: Locator;
  ...
  const yarnBin =
    process.env.COREPACK_ROOT && !ignoreCorepack
  ...
  await Promise.all([
    ...
    ...(YarnVersion !== null
      ? [
          ...
          makePathWrapper(binFolder, `node-gyp` as Filename, process.execPath, [
            yarnBin,
            `run`,
            `--top-level`,
            `node-gyp`,
  ...
  if (locator) {
    if (!project) throw new Error(`Assertion failed: Missing project`);
    ...
    if (workspace) {
      ...
      const linkerOptions = {
        project,
        report: new StreamReport({ stdout: new PassThrough(), configuration: project.configuration }),
      };

      const linker = linkers.find((linker) => linker.supportsPackage(pkg, linkerOptions));
      if (!linker)
        throw new Error(
          `The package ${structUtils.prettyLocator(project.configuration, pkg)} isn't supported by any of the available linkers`,
        );
  ...
  const version =
    YarnVersion !== null ? `yarn/${YarnVersion}` : `yarn/${miscUtils.dynamicRequire(`@yarnpkg/core`).version}-core`;
...
export async function prepareExternalProject(
  cwd: PortablePath,
  outputPath: PortablePath,
  {
    ...
    workspace = null,
    locator = null,
  }: { configuration: Configuration; report: Report; workspace?: string | null; locator?: Locator | null },
) {
  await prepareLimit(async () => {
    await xfs.mktempPromise(async (logDir) => {
      ...
      const { stdout, stderr } = configuration.getSubprocessStreams(logFile, {
        prefix: npath.fromPortablePath(cwd),
        report,
      });

      const devirtualizedLocator =
        locator && structUtils.isVirtualLocator(locator) ? structUtils.devirtualizeLocator(locator) : locator;
      ...
      if (packageManagerSelection !== null) {
        stdout.write(
          `Using ${packageManagerSelection.packageManager} for bootstrap. Reason: ${packageManagerSelection.reason}\n\n`,
        );
      ...
      const ignoreCorepack =
        effectivePackageManager === PackageManager.Yarn2 && !packageManagerSelection?.packageManagerField;
      ...
      await xfs.mktempPromise(async (binFolder) => {
        ...
        const workflows = new Map([
          [
            ...
            async () => {
              ...
              const version = await execUtils.pipevp(
                process.execPath,
                [process.argv[1], `set`, `version`, `classic`, `--only-if-needed`, `--yarn-path`],
                { cwd, env, stdin, stdout, stderr, end: execUtils.EndStrategy.ErrorCode },
              );
              if (version.code !== 0) return version.code;
              ...
              const install = await execUtils.pipevp(`yarn`, [`install`], {
                cwd,
                env,
                stdin,
                stdout,
              ...
              const pack = await execUtils.pipevp(
                `yarn`,
                [...workspaceCli, `pack`, `--filename`, npath.fromPortablePath(outputPath)],
          ...
          [
            ...
            async () => {
              ...
              if (!(await xfs.existsPromise(lockfilePath))) await xfs.writeFilePromise(lockfilePath, ``);
              ...
              const pack = await execUtils.pipevp(
                `yarn`,
                [...workspaceCli, `pack`, `--install-if-needed`, `--filename`, npath.fromPortablePath(outputPath)],
                { cwd, env, stdin, stdout, stderr },
              );
              if (pack.code !== 0) return pack.code;
          ...
          [
            ...
            async () => {
              ...
              if (workspace !== null) {
                ...
                const version = await execUtils.pipevp(`npm`, [`--version`], {
                  cwd,
                  env,
                  stdin,
                  stdout: versionStream,
                  stderr,
                  end: execUtils.EndStrategy.Never,
                });
                versionStream.end();
                ...
                if (!semverUtils.satisfiesWithPrereleases(npmVersion, `>=7.x`)) {
                  ...
                  throw new Error(
                    `Workspaces aren't supported by ${structUtils.prettyDescriptor(configuration, currentNpmDescriptor)}; please upgrade to ${structUtils.prettyDescriptor(configuration, requiredNpmDescriptor)} (npm has been detected as the primary package manager for ${formatUtils.pretty(configuration, cwd, formatUtils.Type.PATH)})`,
                  );
                }
              }
              ...
              const install = await execUtils.pipevp(`npm`, [`install`, `--legacy-peer-deps`], {
                cwd,
                env,
                stdin,
                stdout,
              ...
              const pack = await execUtils.pipevp(`npm`, [`pack`, `--silent`, ...workspaceCli], {
                cwd,
                env,
                stdin,
                stdout: packStream,
        ...
        throw new ReportError(
          MessageName.PACKAGE_PREPARATION_FAILED,
          `Packing the package failed (exit code ${code}, logs can be found here: ${formatUtils.pretty(configuration, logFile, formatUtils.Type.PATH)})`,
        );
      });
    });
  });
...
export async function hasPackageScript(locator: Locator, scriptName: string, { project }: HasPackageScriptOption) {
  ...
  const workspace = project.tryWorkspaceByLocator(locator);
  if (workspace !== null) return hasWorkspaceScript(workspace, scriptName);
...
export async function executePackageScript(
  ...
  return await xfs.mktempPromise(async (binFolder) => {
    const {
      ...
      env,
      cwd: realCwd,
    } = await initializePackageEnvironment(locator, { project, binFolder, cwd, lifecycleScript: scriptName });
...
async function initializeWorkspaceEnvironment(
  workspace: Workspace,
  {
    binFolder,
    cwd,
    lifecycleScript,
  }: { binFolder: PortablePath; cwd?: PortablePath | undefined; lifecycleScript?: string },
) {
...
async function initializePackageEnvironment(
  locator: Locator,
  {
    ...
    cwd,
    lifecycleScript,
  }: { project: Project; binFolder: PortablePath; cwd?: PortablePath | undefined; lifecycleScript?: string },
) {
  // We can avoid using the linkers if the locator is a workspace
  const workspace = project.tryWorkspaceByLocator(locator);
  if (workspace !== null) return initializeWorkspaceEnvironment(workspace, { binFolder, cwd, lifecycleScript });
...
export async function executeWorkspaceLifecycleScript(
  workspace: Workspace,
  lifecycleScriptName: string,
  { cwd, report }: ExecuteWorkspaceLifecycleScriptOptions,
) {
...
export async function maybeExecuteWorkspaceLifecycleScript(
  workspace: Workspace,
  lifecycleScriptName: string,
...
export function isNodeScript(p: PortablePath) {
  ...
  if (ext.match(/\.[cm]?[jt]sx?$/)) return true;

  if (ext === `.exe` || ext === `.bin`) return false;
 ...
 */
export async function getPackageAccessibleBinaries(
  locator: Locator,
  { project }: GetPackageAccessibleBinariesOptions,
): Promise<PackageAccessibleBinaries> {
  const configuration = project.configuration;
  ...
  for (const descriptor of pkg.dependencies.values()) {
    ...
    if (!resolution)
      throw new Error(
        `Assertion failed: The resolution (${structUtils.prettyDescriptor(configuration, descriptor)}) should have been registered`,
      );

    visibleLocators.add(resolution);
  }

  const dependenciesWithBinaries = await Promise.all(
    Array.from(visibleLocators, async (locatorHash) => {
      ...
      if (dependency.bin.size === 0) return miscUtils.mapAndFilter.skip;

      const linker = linkers.find((linker) => linker.supportsPackage(dependency, linkerOptions));
      if (!linker) return miscUtils.mapAndFilter.skip;
  ...
  for (const candidate of dependenciesWithBinaries) {
    if (candidate === miscUtils.mapAndFilter.skip) continue;
...
export async function executeWorkspaceAccessibleBinary(
  ...
  binaryName: string,
  args: Array<string>,
  { cwd, stdin, stdout, stderr, packageAccessibleBinaries }: ExecuteWorkspaceAccessibleBinaryOptions,
) {
