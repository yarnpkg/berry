...
export async function request(target: string, body: Body, {configuration, headers, json, jsonRequest = json, jsonResponse = json, method = Method.GET}: Options) {
  ...
  const noProxy = configuration.get(`noProxy`);
  const agent = new GateAgent({httpProxy, httpsProxy, noProxy, keepAlive: true});

  const gotOptions: ExtendOptions = {agent: {http: agent, https: agent}, headers, method};
