...
function parseProxy(specifier: string) {
  ...
  if (url.username && url.password) proxy.proxyAuth = `${url.username}:${url.password}`;
...
function prettyResponseCode({ statusCode, statusMessage }: Response, configuration: Configuration) {
  ...
  return formatUtils.applyHyperlink(
    configuration,
    `${prettyStatusCode}${statusMessage ? ` (${statusMessage})` : ``}`,
    href,
  );
}

async function prettyNetworkError(
        ...
        report.reportError(
          ...
          `  ${formatUtils.prettyField(configuration, {
            ...
            value: formatUtils.tuple(
              ...
              formatUtils.prettyList(configuration, err.request.redirects, formatUtils.Type.URL),
            ),
          })}`,
        );
      }
      ...
      if (err.request.retryCount === err.request.options.retry.limit) {
        report.reportError(
          ...
          `  ${formatUtils.prettyField(configuration, {
            ...
            value: formatUtils.tuple(
              formatUtils.Type.NO_HINT,
              `${formatUtils.pretty(configuration, err.request.retryCount, formatUtils.Type.NUMBER)} (can be increased via ${formatUtils.pretty(configuration, `httpRetry`, formatUtils.Type.SETTING)})`,
            ),
          })}`,
        );
      }
...
export type Body = { [key: string]: any } | string | Buffer | null;
...
export async function request(
  ...
  body: Body,
  {
    ...
    method = Method.GET,
    wrapNetworkRequest,
  }: Omit<Options, "customErrorMessage">,
) {
  ...
  const wrappedRequest =
    typeof wrapNetworkRequest !== `undefined` ? await wrapNetworkRequest(realRequest, options) : realRequest;

  const executor = await configuration.reduceHook(
    (hooks) => {
...
export async function get(
  target: string,
  { configuration, jsonResponse, customErrorMessage, wrapNetworkRequest, ...rest }: Options,
) {
  const runRequest = () =>
    prettyNetworkError(request(target, null, { configuration, wrapNetworkRequest, ...rest }), {
      configuration,
      customErrorMessage,
...
export async function put(target: string, body: Body, { customErrorMessage, ...options }: Options): Promise<Buffer> {
  const response = await prettyNetworkError(request(target, body, { ...options, method: Method.PUT }), {
    customErrorMessage,
...
export async function del(target: string, { customErrorMessage, ...options }: Options): Promise<Buffer> {
  const response = await prettyNetworkError(request(target, null, { ...options, method: Method.DELETE }), {
    customErrorMessage,
    configuration: options.configuration,
  });
...
async function requestImpl(
  target: string | URL,
  body: Body,
  { configuration, headers, jsonRequest, jsonResponse, method = Method.GET }: Omit<Options, "customErrorMessage">,
): Promise<Response> {
  const url = typeof target === `string` ? new URL(target) : target;
  ...
  if (networkConfig.enableNetwork === false)
    throw new ReportError(
      ...
      `Request to '${url.href}' has been blocked because of your configuration settings`,
    );

  if (url.protocol === `http:` && !micromatch.isMatch(url.hostname, configuration.get(`unsafeHttpWhitelist`)))
    throw new ReportError(
      MessageName.NETWORK_UNSAFE_HTTP,
      `Unsafe http requests must be explicitly whitelisted in your configuration (${url.hostname})`,
    );

  const agent = {
    http: networkConfig.httpProxy ? tunnel.httpOverHttp(parseProxy(networkConfig.httpProxy)) : globalHttpAgent,
