...
export function mapAndFilter<In, Out>(iterableIterator: IterableIterator<In>, cb: (value: In) => Out | typeof mapAndFilterSkip): Array<Out> {
...
export function mapAndFind<In, Out>(iterableIterator: IterableIterator<In>, cb: (value: In) => Out | typeof mapAndFindSkip): Out | undefined {
  for (const value of iterableIterator) {
    const out = cb(value);
