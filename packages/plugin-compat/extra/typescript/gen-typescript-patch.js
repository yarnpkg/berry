...
async function fetch(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, (res) => {
      if (!(res.statusCode >= 200 && res.statusCode < 300)) throw new Error(`Failed to fetch ${url}`);
...
async function fetchVersions(range) {
  if (typeof relevantVersions === `undefined`) {
    ...
    for (const version of allVersions) {
      if (IGNORED_VERSIONS.has(version)) continue;

      const pre = semver.prerelease(version);
      if (pre && pre[0] !== `beta` && pre[0] !== `rc`) continue;
  ...
  for (const version of relevantVersions) {
    ...
    if (!semver.satisfies(base, range)) continue;
...
async function buildRepository({ from, to, onto }) {
  ...
  await execFile(
    fs.existsSync(`${TS_REPO}/node_modules/.bin/hereby`) ? `./node_modules/.bin/hereby` : `./node_modules/.bin/gulp`,
    [`local`, `LKG`],
    TS_REPO_SPAWN,
  );
  ...
  if (stat.size < 100000) throw new Error(`Something is wrong; typescript.js got generated with a stupid size`);
...
async function run({ from, to, onto, range }) {
  const hash = crypto.createHash(`md5`).update(JSON.stringify({ from, to, onto })).digest(`hex`);
...
async function validate(version, patch) {
  ...
  if (!fs.existsSync(path.join(tmpDir, `package`))) await execFile(`tar`, [`xvf`, tarball], { cwd: tmpDir });
