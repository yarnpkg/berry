...
export async function makeArchiveFromDirectory(
  source: string,
  {baseFs = new NodeFS(), prefixPath = `/`}: MakeArchiveFromDirectoryOptions = {},
): Promise<ZipFS> {
...
export async function makeArchive(
  tgz: Buffer,
  {stripComponents = 0, prefixPath = `.`}: MakeArchiveOptions = {},
): Promise<ZipFS> {
  const zipFs = new ZipFS(tmpNameSync(), {create: true});
  ...
  parser.on(`entry`, (entry: any) => {
    ...
    if (entry.type === `Directory` || (entry.mode & 0o111) !== 0) mode |= 0o111;
