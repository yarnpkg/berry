...
export class LockfileResolver implements Resolver {
  supportsDescriptor(descriptor: Descriptor, opts: MinimalResolveOptions) {
    ...
    if (opts.project.storedPackages.has(structUtils.convertDescriptorToLocator(descriptor).locatorHash)) return true;

    return false;
  }

  supportsLocator(locator: Locator, opts: MinimalResolveOptions) {
    if (opts.project.storedPackages.has(locator.locatorHash)) return true;
