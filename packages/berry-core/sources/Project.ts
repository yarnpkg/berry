...
export class Project {
  ...
  private async setupResolutions() {
    ...
    const lockfilePath = portablePathUtils.join(this.cwd, this.configuration.get(`lockfileFilename`));
  ...
  tryWorkspaceByCwd(workspaceCwd: PortablePath) {
    if (!portablePathUtils.isAbsolute(workspaceCwd))
      workspaceCwd = portablePathUtils.resolve(this.cwd, workspaceCwd);
  ...
  async linkEverything({cache, report}: InstallOptions) {
    ...
    await xfs.mkdirpPromise(portablePathUtils.dirname(bstatePath));
