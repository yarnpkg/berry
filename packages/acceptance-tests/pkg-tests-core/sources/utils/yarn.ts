...
export async function readConfiguration(
  dir: PortablePath,
  { filename = DEFAULT_RC_FILENAME }: { filename?: Filename } = {},
) {
...
export async function readManifest(
  dir: PortablePath,
  { key, filename = Filename.manifest }: { key?: keyof Manifest; filename?: Filename } = {},
) {
...
export async function writeManifest(
  dir: PortablePath,
  value: { [key: string]: any },
  { filename = Filename.manifest }: { filename?: Filename } = {},
) {
  return await fsUtils.writeJson(ppath.join(dir, filename), value);
}
