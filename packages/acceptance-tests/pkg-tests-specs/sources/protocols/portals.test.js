      ...
      makeTemporaryEnv(
        ...
        async ({ path, run, source }) => {
          await xfs.copyPromise(
            `${path}/provides-peer-deps-1-0-0`,
            await getPackageDirectoryPath(`provides-peer-deps-1-0-0`, `1.0.0`),
          );
          await xfs.copyPromise(
            `${path}/provides-peer-deps-2-0-0`,
            await getPackageDirectoryPath(`provides-peer-deps-2-0-0`, `1.0.0`),
          );

          const providesPeerDeps100 = await xfs.readJsonPromise(`${path}/provides-peer-deps-1-0-0/package.json`);
          providesPeerDeps100.dependencies[`peer-deps`] =
            `portal:${await getPackageDirectoryPath(`peer-deps`, `1.0.0`)}`;
          ...
          providesPeerDeps200.dependencies[`peer-deps`] =
            `portal:${await getPackageDirectoryPath(`peer-deps`, `1.0.0`)}`;
