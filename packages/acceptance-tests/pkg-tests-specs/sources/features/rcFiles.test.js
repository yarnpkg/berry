  ...
  describe(`Rc files`, () => {
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        ...
        expect(parseJsonStream((await run(`config`, `--json`)).stdout, `key`)).toMatchObject({
          [`pnpShebang`]: {
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        ...
        expect(
          parseJsonStream((await run(`config`, `--json`, { rcFilename: `.foobarrc` })).stdout, `key`),
        ).toMatchObject({
          [`pnpShebang`]: {
            effective: `Hello World!`,
          },
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        await xfs.writeFilePromise(`${path}/.yarnrc.yml`, `unknownConfigKey: 42\n`);
