    ...
    test(
      ...
      makeTemporaryEnv(
        ...
        async ({path, run, source}) => {
          ...
          expect(stdout).toContain(`provides no-deps with version 1.1.0 which doesn't satisfy what mismatched-peer-deps-lvl0@npm:1.0.0 and its descendants request (run yarn explain [...] for details)`);
        },
      ),
    );
  });
