...
describe(`Cache`, () => {
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            if (!withLockfile) await xfs.removePromise(ppath.join(path, Filename.lockfile));
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        const cacheFile = ppath.join(
          path,
          `.yarn/global/cache`,
          cacheFiles.find((name) => name.startsWith(`no-deps-`))!,
        );
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        const otherCacheFile = ppath.join(
          path,
          `.yarn/cache`,
          otherCacheFiles.find((name) => name.startsWith(`no-deps-`))!,
        );
  ...
  test(
    `it should not confuse the cache key when merging an upgraded cache key branch into a feature branch`,
    makeTemporaryEnv(
      {
        dependencies: {
