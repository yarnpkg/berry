...
describe(`Features`, () => {
  describe(`Plugins`, () => {
    test(
      `it should properly load a plugin via the local rc file`,
    ...
    test(
      `it should accept asynchronous plugins`,
      makeTemporaryEnv({}, async ({ path, run, source }) => {
    ...
    test(
      `it should properly load multiple plugins via the local rc file, in the right order`,
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        await xfs.writeFilePromise(`${path}/plugin-a.js` as PortablePath, COMMANDS_PLUGIN(`A`, { printOnBoot: true }));
    ...
    test(
      `it should be able to define the configuration parameters of official plugins`,
      makeTemporaryEnv({}, async ({ path }) => {
        await xfs.writeFilePromise(
          `${path}/.yarnrc.yml` as PortablePath,
          stringifySyml({
            plugins: [`./plugin-a.js`],
    ...
    test(
      `it should be able to define the configuration parameters of third-party plugins`,
      makeTemporaryEnv({}, async ({ path }) => {
        await xfs.writeFilePromise(
          `${path}/.yarnrc.yml` as PortablePath,
          stringifySyml({
    ...
    test(
      `it should fetch missing plugins`,
      makeTemporaryEnv({}, async ({ path, run, source }) => {
    ...
    test(
      `it should throw an error when fetching the plugin but the checksum does not match`,
