...
describe(`Features`, () => {
  describe(`Editor SDK`, () => {
     ...
     */
    test(`it should patch message into VSCode typescript language extension for zip schemes`, async () => {
      const child = spawn(
        process.execPath,
        [require.resolve(`@yarnpkg/monorepo/.yarn/sdks/typescript/lib/tsserver.js`)],
        {
      ...
      try {
        ...
        const lodashTypeDef = require.resolve(`@types/lodash/index.d.ts`).replace(/\\/g, `/`).replace(/^\/?/, `/`);
        ...
        const ourUtilityFile = require.resolve(`./editorSdks.utility.ts`).replace(/\\/g, `/`).replace(/^\/?/, `/`);

        // Same thing, but this file has virtual instances.
        const yarnpkgCli = npath
          .normalize(npath.join(__dirname, `../../../../yarnpkg-cli/sources/index.ts`))
          .replace(/\\/g, `/`);
