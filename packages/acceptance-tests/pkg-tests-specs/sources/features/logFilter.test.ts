...
const makeCodeFilter = (level: string) =>
  JSON.stringify([
    {
      code: `YN0005`,
      level,
...
const makeTextFilter = (level: string) =>
  JSON.stringify([
    {
...
const makePatternFilter = (level: string) =>
  JSON.stringify([
    {
...
describe(`Features`, () => {
  describe(`LogFilters`, () => {
    test(
      `it should allow to filter by message name`,
      makeTemporaryEnv(
        {
          dependencies: {
    ...
    test(
      `it should allow to filter by message text`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps-scripted`]: `1.0.0`,
    ...
    test(
      `it should allow to filter by message text with colors enabled`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps-scripted`]: `1.0.0`,
    ...
    test(
      `it should allow to filter by pattern`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps-scripted`]: `1.0.0`,
    ...
    test(
      `it should match any part of the log entry using patterns`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps`]: `1.0.0`,
    ...
    test(
      `it should not print the project validation timer header and footer if its contents are discarded via logFilters`,
    ...
    test(
      `it should not print the peer dependencies footer if the section contents are discarded via logFilters`,
      makeTemporaryEnv(
        {
