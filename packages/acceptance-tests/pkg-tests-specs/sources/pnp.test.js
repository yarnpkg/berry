...
describe(`Plug'n'Play`, () => {
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      api.resolveToUnqualified(
        `${npath.fromPortablePath(path)}/.pnp.cjs`,
        `${npath.fromPortablePath(path)}/some/path/that/doesnt/exists/please/`,
      );
    }),
  );
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await writeFile(`${path}/packages/workspace-a/index.js`, `module.exports = require('no-deps');`);
  ...
  test(
    ...
    makeTemporaryEnv({ dependencies: { [`various-requires`]: `1.0.0` } }, async ({ path, run, source }) => {
      await run(`install`);
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        const dependencyMessage = await source(
          `{ try { require('various-requires/invalid-require') } catch (error) { return error.message } }`,
        );
  ...
  test(
    ...
    makeTemporaryEnv({ dependencies: { [`peer-deps`]: `1.0.0` } }, async ({ path, run, source }) => {
      await run(`install`);
  ...
  test(
    ...
    makeTemporaryEnv({ dependencies: { [`broken-peer-deps`]: `1.0.0` } }, async ({ path, run, source }) => {
      await run(`install`);
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          source(
            `require('provides-peer-deps-1-0-0').dependencies['peer-deps'] === require('provides-peer-deps-2-0-0').dependencies['peer-deps']`,
          ),
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          source(
            `require('provides-peer-deps-1-0-0').dependencies['peer-deps'] === require('provides-peer-deps-1-0-0-too').dependencies['peer-deps']`,
          ),
        ).resolves.toEqual(true);
      },
    ),
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          source(
            `require('forward-peer-deps').dependencies['peer-deps'] === require('forward-peer-deps-too').dependencies['peer-deps']`,
          ),
        ).resolves.toEqual(true);
      },
    ),
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await expect(source(`require('peer-deps') === require('peer-deps-too')`)).resolves.toEqual(false);
      },
    ),
  );
  ...
  test(
    ...
    makeTemporaryEnv({ dependencies: { [`no-deps`]: `1.0.0` } }, async ({ path, run, source }) => {
      await run(`install`);
      ...
      async ({ path, run, source }) => {
        ...
        await makeTemporaryEnv(
          ...
          async ({ path: path2, run: run2, source: source2 }) => {
            ...
            await expect(xfs.readFilePromise(`${path2}/.pnp.cjs`, `utf8`)).resolves.toEqual(
              await xfs.readFilePromise(`${path}/.pnp.cjs`, `utf8`),
            );
          },
        )();
      },
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      await expect(
        run(`node`, `-e`, `console.log(21);`, {
          env: { NODE_OPTIONS: `--require ${JSON.stringify(npath.join(npath.fromPortablePath(path), `foo`))}` },
        }),
      ).resolves.toMatchObject({
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      await expect(xfs.readdirPromise(ppath.join(path, `node_modules`))).resolves.toEqual([`.cache`]);
    }),
  );
  ...
  test(
    `it should NOT remove lingering node_modules inside folders matched by pnpIgnorePatterns`,
    makeTemporaryEnv(
      {
        workspaces: [`foo`],
      },
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      cp.execFileSync(`node`, [npath.fromPortablePath(`${tmp}/index.js`), npath.fromPortablePath(`${path}/.pnp.cjs`)], {
        encoding: `utf-8`,
      });
    }),
  );
  ...
  test(
    `should skip building incompatible package`,
    makeTemporaryEnv(
      {
        private: true,
        dependencies: {
      ...
      async ({ path, run, source }) => {
        ...
        expect(stdout).toMatch(
          new RegExp(
            `dep@file:./dep.*The ${process.platform}-${process.arch}(-[a-z]+)? architecture is incompatible with this package, build skipped.`,
          ),
        );
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      await expect(source(`require.resolve('./foo/')`)).resolves.toEqual(
        npath.fromPortablePath(`${path}/foo/index.js`),
      );
    }),
  );
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      await expect(source(`require.resolve('pkg')`)).resolves.toEqual(
        npath.fromPortablePath(`${path}/package/index.js`),
      );
    }),
  );
  ...
  test(
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      await xfs.mktempPromise(async (secondProject) => {
        ...
        await xfs.writeFilePromise(
          `${secondProject}/index.js`,
          `module.exports = require.resolve('no-deps', {paths: [require.resolve('no-deps')]})`,
        );
  ...
  test(
    ...
    makeTemporaryEnv(
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          source(`require('module').findPnpApi(require('path').resolve('ignored/node_modules/.cache'))`),
        ).resolves.toBe(null);
      },
    ),
  );
    ...
    makeTemporaryEnv({}, async ({ path, run, source }) => {
      ...
      await expect(run(`node`, `./index.js`)).resolves.toMatchObject({
        ...
        stderr: expect.stringContaining(
          `[Warning] The runtime detected new information in a PnP file; reloading the API instance`,
        ),
      });
