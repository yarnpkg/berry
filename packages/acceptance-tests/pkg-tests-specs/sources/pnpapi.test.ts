  ...
  describe(`std - v1`, () => {
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        ...
        await expect(
          source(
            `require('pnpapi').findPackageLocator(${JSON.stringify(`${npath.fromPortablePath(ppath.dirname(path))}/`)})`,
          ),
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            ).resolves.toEqual([[`no-deps`, `npm:1.0.0`]]);
          },
        ),
      );
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            ).resolves.toEqual([`@types/no-deps`, `no-deps`]);
          },
        ),
      );
    ...
    describe(`resolveRequest`, () => {
      test(
        ...
        makeTemporaryEnv({}, async ({ path, run, source }) => {
          ...
          ).resolves.toEqual(npath.fromPortablePath(`${path}/.pnp.cjs`));
        }),
      );
      ...
      test(
        ...
        makeTemporaryEnv({}, async ({ path, run, source }) => {
          ...
          await expect(
            source(
              `require('pnpapi').resolveRequest('node:fs', ${JSON.stringify(`${npath.fromPortablePath(path)}/`)})`,
            ),
          ).resolves.toEqual(null);
        }),
      );
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await expect(
              source(
                `require('pnpapi').resolveRequest('fs', ${JSON.stringify(`${npath.fromPortablePath(path)}/`)}, {considerBuiltins: false})`,
              ),
            ).resolves.toEqual(npath.fromPortablePath(`${path}/fs/index.js`));
          },
        ),
      );
      ...
      test(
        ...
        makeTemporaryEnv({}, async ({ path, run, source }) => {
          ...
          await expect(
            source(
              `require('pnpapi').resolveRequest('./foo', ${JSON.stringify(`${npath.fromPortablePath(path)}/`)}, {extensions: ['.bar']})`,
            ),
          ).resolves.toEqual(npath.fromPortablePath(`${path}/foo.bar`));
        }),
      );
    });
  });
  ...
  describe(`std - v2`, () => {
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        ...
        await expect(
          source(`(() => require('pnpapi').resolveRequest('no-deps', ${JSON.stringify(`${path}/`)}))()`),
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await expect(
              source(
                `require('pnpapi').getPackageInformation({name: 'no-deps', reference: ${JSON.stringify(reference)}}).linkType`,
              ),
            ).resolves.toEqual(`HARD`);
          },
        ),
      );
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await expect(
              source(
                `require('pnpapi').getPackageInformation({name: 'self', reference: ${JSON.stringify(reference)}}).linkType`,
              ),
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await expect(source(`require('pnpapi').getDependencyTreeRoots()`)).resolves.toEqual([
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await xfs.writeJsonPromise(`${path}/packages/workspace-b/package.json` as PortablePath, {
              name: `workspace-b`,
            });
      ...
      test(
        ...
        makeTemporaryEnv(
          ...
          async ({ path, run, source }) => {
            ...
            await expect(source(`require('pnpapi').resolveVirtual(require.resolve('no-deps'))`)).resolves.toEqual(null);
          },
        ),
      );
        ...
        await expect(
          ...
          externalException: {
            message: expect.stringContaining(
              `Your application tried to access fs. While this module is usually interpreted as a Node builtin,`,
            ),
