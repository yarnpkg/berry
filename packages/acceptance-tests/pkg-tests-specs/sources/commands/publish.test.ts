...
describe(`publish`, () => {
  ...
  test(
    `should fail when invalid otp is given`,
    makeTemporaryEnv(
      {
        name: `otp-required`,
        version: `1.0.0`,
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          run(`npm`, `publish`, `--otp`, `invalid_otp`, {
            env: {
  ...
  test(
    `should accept an otp and skip prompting for it`,
    makeTemporaryEnv(
      {
        name: `otp-required`,
        version: `1.0.0`,
      ...
      async ({ path, run, source }) => {
        ...
        await expect(
          run(`npm`, `publish`, `--otp`, validLogins.otpUser.npmOtpToken, {
            env: {
              YARN_NPM_AUTH_TOKEN: validLogins.otpUser.npmAuthToken,
            },
          }),
  ...
  test(
    `should publish a package with the readme content`,
    makeTemporaryEnv(
      {
