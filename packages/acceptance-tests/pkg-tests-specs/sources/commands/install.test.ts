        ...
        async ({ path, run, source }) => {
          ...
          // Then we disguise 2.0.0 as 1.0.0. The stored checksum will stay the same.
          {
            ...
            await xfs.movePromise(
              ppath.join(path, `.yarn/cache`, archiveName2),
              ppath.join(path, `.yarn/cache`, archiveName1),
            );
          }
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        ...
        await expect(run(`install`, { cwd: ppath.join(path, `non-workspace`) })).rejects.toMatchObject({
          ...
          stdout: expect.stringMatching(
            /The nearest package directory \(.+\) doesn't seem to be part of the project declared in .+\./g,
          ),
        });
      }),
    );
    ...
    test(
      ...
      makeTemporaryEnv(
        ...
        async ({ path, run, source }) => {
          ...
          await expect(
            tests.startRegistryRecording(async () => {
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run }) => {
        await expect(run(`install`, `--mode=update-lockfile`, `--immutable`)).rejects.toMatchObject({
          code: 1,
          stdout: expect.stringMatching(
            /--immutable and --immutable-cache cannot be used with --mode=update-lockfile/g,
          ),
        });
        ...
        await expect(
          run(`install`, `--mode=update-lockfile`, `--immutable`, `--immutable-cache`),
