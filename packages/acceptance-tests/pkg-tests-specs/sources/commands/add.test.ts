...
describe(`Commands`, () => {
  describe(`add`, () => {
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        await run(`add`, `no-deps`);
        await expect(run(`add`, `no-deps`, `-D`)).rejects.toThrowError(/already listed as a regular dependency/);
      }),
    );
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        await run(`add`, `no-deps`);
        await expect(run(`add`, `no-deps`, `-P`)).rejects.toThrowError(/already listed as a regular dependency/);
      }),
    );
    ...
    test(
      ...
      makeTemporaryEnv({}, async ({ path, run, source }) => {
        await run(`add`, `no-deps`, `-P`);
        await expect(run(`add`, `no-deps`)).rejects.toThrowError(/already listed as a peer dependency/);
      }),
    );
    ...
    test(
      `it should clean the cache when cache lives inside the project`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps`]: `1.0.0`,
    ...
    test(
      `it should not clean the cache when cache lives inside the project but global cache is set`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps`]: `1.0.0`,
    ...
    test(
      `it should not clean the cache when cache lives outside the project`,
      makeTemporaryEnv(
        {
          dependencies: {
            [`no-deps`]: `1.0.0`,
