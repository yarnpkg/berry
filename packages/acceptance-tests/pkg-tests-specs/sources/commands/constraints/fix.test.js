...
describe(`Commands`, () => {
  ...
  describe(`constraints --fix`, () => {
    it(
      `shouldn't crash due to an unending fix loop`,
      makeTemporaryEnv(
        {
          foo: ``,
        },
      ...
      `,
          ...
          await run(`constraints`, `--fix`, {
            execArgv: [
              `--require`,
              require.resolve(`@yarnpkg/monorepo/.pnp.cjs`),
              `--require`,
              require.resolve(`@yarnpkg/monorepo/scripts/setup-ts-execution`),
              `--require`,
              require.resolve(`@yarnpkg/monorepo/scripts/detect-unsafe-writes.ts`),
            ],
          });
    ...
    test(
      `test apply fix to dependencies`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await xfs.writeFilePromise(
          `${path}/constraints.pro`,
          `
    ...
    test(
      `test apply fix to fields and manifests`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await xfs.writeFilePromise(
          `${path}/constraints.pro`,
          `
    ...
    test(
      `test applying fix shouldn't duplicate workspaces`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await xfs.writeFilePromise(
          `${path}/constraints.pro`,
          `
    ...
    it(
      `should preserve the raw manifest data when applying a fix`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await xfs.writeFilePromise(
          `${path}/constraints.pro`,
          `
    ...
    test(
      `test apply fix to string fields`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
    ...
    test(
      `test apply fix to object fields`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await environments[`various field types`](path);
    ...
    test(
      `test apply fix to array fields`,
      makeTemporaryEnv(manifest, async ({ path, run, source }) => {
        await environments[`various field types`](path);
      ...
      `,
        ...
        expect(fixedManifest._files).toStrictEqual([`/a`, `/b`, `/c`]);
      }),
    );
  });
});
