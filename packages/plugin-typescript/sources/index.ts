...
const afterWorkspaceDependencyAddition = async (
  ...
  const typesName = descriptor.scope ? `${descriptor.scope}__${descriptor.name}` : `${descriptor.name}`;
  ...
  const suggestions = await suggestUtils.getSuggestedDescriptors(request, null, {
    project,
    cache,
    target,
    modifier,
