...
import {MiniZipFS}                                    from './MiniZipFS';
/**
...
export type ZipOpenFSOptions = Omit<MountFSOptions<MiniZipFS>,
  | `factoryPromise`
  | `factorySync`
  | `getMountPoint`
> & {
...
export class MiniZipOpenFS extends MountFS<MiniZipFS> {
  static async openPromise<T>(fn: (zipOpenFs: MiniZipOpenFS) => Promise<T>, opts?: ZipOpenFSOptions): Promise<T> {
    const zipOpenFs = new MiniZipOpenFS(opts);
  ...
  constructor(opts: ZipOpenFSOptions = {}) {
    if (opts.readOnlyArchives !== true) {
      throw new Error(`The MiniZipOpenFS can only be used with read-only archives`);
    }
    ...
    const factoryPromise = async (baseFs: FakeFS<PortablePath>, p: PortablePath) => {
      ...
      return () => {
        return new MiniZipFS(p, zipOptions);
      };
    };
