...
If you've run `yarn install` and it still isn't working, this may indicate that you've accidentally created a `package.json` in one of the parent directories of the project, or your home directory.
This can happen if, for example, you ran `yarn add react` while your terminal's working directory was not in your project, but rather in your home directory.
To remedy this, delete the `package.json` file in your home directory if it exists.
