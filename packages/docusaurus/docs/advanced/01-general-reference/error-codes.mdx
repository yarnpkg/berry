...
## YN0093 - `UNPUBLISHABLE_DEPENDENCY`

A non-private workspace lists a dependency that won't work once the package is published, so packing it was aborted.

This currently applies to the `patch:` protocol. The patch file is resolved relative to your project and isn't shipped alongside the package, so installs from the registry would fail. To patch a dependency of a published package, apply the patch through the [`resolutions`](/configuration/manifest#resolutions) field of your project root instead, and keep a regular semver range in the published package.
