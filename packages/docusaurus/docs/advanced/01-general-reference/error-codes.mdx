...
## YN0093 - `PEER_DEPENDENCY_PROVIDED_BY_DEV_DEPENDENCY`

A workspace provides a peer dependency only through `devDependencies`, or a workspace peer request is satisfied by the requester's own `devDependencies`.

This may work during local installs because Yarn can see the workspace dev dependency, but the dependency won't be available once the workspace is packed, published, or installed in production. Move the dependency to regular `dependencies`, or make the peer optional if it isn't required in production contexts.
