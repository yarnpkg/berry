...
export type ScriptLine = RawLine | CommandLine;
...
const makeCommandLine = (line: string, cli: YarnCli) => {
  ...
  if (parsed.type !== `command`) throw new Error(`Unsupported command type: "${parsed.type}" when parsing "${line}"`);

  const strArgs = parsed.args.map((arg, index) => {
    if (arg.type !== `argument`) throw new Error(`Unsupported argument type: "${arg.type}" when parsing "${line}"`);
  ...
  const tooltip = definition?.description ? capitalize(definition.description) : null;
...
export const plugin = () => () => {
  ...
  const transformer = async (ast) => {
    ...
    visit(ast, (node, ancestors) => {
      if (node.type !== `inlineCode` && (node.type !== `code` || node.lang)) return;
      ...
      highlightNodes.push(
        Promise.all(lines.map((line) => line())).then((lines) => {
