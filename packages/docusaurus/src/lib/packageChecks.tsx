...
export const checks: Array<Check> = [
  {
    ...
    useCheck: ({ name, version }) => {
      ...
      if (releaseInfo.npm.deprecated) return { ok: false };
  ...
  },
  {
    ...
    useCheck: ({ name, version }) => {
      ...
      if (resolution.endsWith(`.js`) && releaseInfo.npm.type === `module`) return { ok: false };
  ...
  },
  {
    ...
    useCheck: ({ name, version }) => {
      ...
      for (const name of [`preinstall`, `install`, `postinstall`])
        if (releaseInfo.npm.scripts?.[name]) return { ok: false };
  ...
  },
  {
    ...
    useCheck: ({ name, version }) => {
      ...
      const extensions = [...STANDARD_EXTENSIONS, ...tsExtensions];
      ...
      for (const ext of tsExtensions) if (releaseInfo.jsdelivr.fileSet.has(`${fileNoExt}${ext}`)) return { ok: true };

      if (dtPackageName && dtPackage.data) {
        const search = new URLSearchParams(location.search);

        for (const key of search.keys()) if (key !== `q`) search.delete(key);
        ...
        return {
          ...
          message: (
            <>
              Types are available via <Link href={href}>{dtPackageName}</Link>
            </>
          ),
        };
      }
