...
const getTooltip = ({ tooltip }: { tooltip: string | null }) =>
  tooltip
    ? ({
...
function RawLine({ line }: { line: RawLine }) {
  return <div className={styles.comment}>{line.value}</div>;
}
...
function CommandLine({ line }: { line: CommandLine }) {
  ...
  const renderTokens = (start: number, end: number) => {
    return line.tokens.slice(start, end).map((token, index) => (
      <React.Fragment key={index}>
        {start + index > 0 &&
        token.segmentIndex !== line.tokens[start + index - 1].segmentIndex &&
  ...
  const wrapPath: (children: React.ReactNode) => React.ReactNode = line.tooltip
    ? (children) => (
        <Link
          ...
          href={`/cli/${line.command.path.join(`/`)}`}
          {...getTooltip(line)}
          children={children}
        />
      )
