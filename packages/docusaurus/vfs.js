...
for (const [position, { binary, package }] of binaries.entries()) {
  ...
  const docUrl = isMainPackage ? `/cli` : `/${name}/cli`;
  ...
  for (const command of commands) {
    ...
    if (!isMainPackage) pathSegments.unshift(`yarn`);
  ...
  for (let t = 0; t < commands.length; ++t) {
    ...
    if (command.options.length > 0) {
      const addAnchor = (definition) =>
        `<h4 id="${encodeURIComponent(`options-${definition}`.replace(/-+/g, `-`))}" className="header-code"><code className="language-text">${definition}</code></h4>`;
      sections.push(
        [
          `## Options\n`,
          `\n`,
          ...
          `| ---------- | ----------- |\n`,
          ...command.options.map(({ definition, description }) => `| ${addAnchor(definition)} | ${description} |\n`),
          `\n`,
        ].join(``),
      );
    }
