...
export function makeSpec({
  ...
  const folderHashSpread = typeof hash !== `undefined` ? { hash } : ({} as {});
...
export function makeLocator(
  ident: Ident,
  { parentLocator, path, hash, protocol }: Parameters<typeof makeSpec>[number],
): Locator {
...
export async function makeArchiveFromLocator(
  locator: Locator,
  { protocol, fetchOptions, inMemory = false }: { protocol: string; fetchOptions: FetchOptions; inMemory?: boolean },
): Promise<ZipFS> {
...
export async function makeBufferFromLocator(
  locator: Locator,
  { protocol, fetchOptions }: { protocol: string; fetchOptions: FetchOptions },
) {
