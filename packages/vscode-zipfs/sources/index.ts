...
function unmount(uri: vscode.Uri): void {
  ...
  if (!vscode.workspace.workspaceFolders) throw new Error(`Assertion failed: workspaceFolders is undefined`);
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(`zipfs.unmountZipFile`, (uri: vscode.Uri) => {
      unmount(uri);
    }),
  );
