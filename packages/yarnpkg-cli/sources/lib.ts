...
function runYarnPath(cli: YarnCli, argv: Array<string>, { yarnPath }: { yarnPath: PortablePath }) {
  if (!xfs.existsSync(yarnPath)) {
    cli.error(
      new Error(`The "yarn-path" option has been set, but the specified location doesn't exist (${yarnPath}).`),
    );
    return 1;
  }
...
function initTelemetry(cli: YarnCli, { configuration }: { configuration: Configuration }) {
  ...
  for (const name of configuration.plugins.keys())
    if (pluginCommands.has(name.match(PLUGIN_REGEX)?.[1] ?? ``)) Configuration.telemetry?.reportPluginName(name);
...
async function run(
  cli: YarnCli,
  argv: Array<string>,
  { selfPath, pluginConfiguration }: { selfPath: PortablePath | null; pluginConfiguration: PluginConfiguration },
) {
