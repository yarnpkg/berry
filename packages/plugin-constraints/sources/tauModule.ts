...
function prependGoals(thread: pl.type.Thread, point: pl.type.State, goals: Array<pl.type.Term<number, string>>): void {
  thread.prepend(goals.map((goal) => new pl.type.State(point.goal.replace(goal), point.substitution, point)));
}
  ...
  `constraints`,
  {
      ...
      if (isVariable(workspaceCwd)) {
        if (workspace !== null) {
          prependGoals(thread, point, [
            new pl.type.Term(`=`, [workspaceCwd, new pl.type.Term(String(workspace.relativeCwd))]),
          ]);
        }
      }
    ...
    [`workspace_field_test/3`]: (thread, point, atom) => {
      ...
      thread.prepend([
        new pl.type.State(
          point.goal.replace(
            new pl.type.Term(`workspace_field_test`, [workspaceCwd, fieldName, checkCode, new pl.type.Term(`[]`, [])]),
          ),
          point.substitution,
          point,
        ),
    ...
    [`workspace_field_test/4`]: (thread, point, atom) => {
      ...
      for (const [index, value] of (checkArgv.toJavaScript() as Array<string>).entries()) vars[`$${index}`] = value;
