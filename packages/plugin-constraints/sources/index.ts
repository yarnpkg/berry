...
const plugin: Plugin<Hooks> = {
  ...
  commands: [ConstraintsQueryCommand, ConstraintsSourceCommand, ConstraintsCheckCommand],
  hooks: {
    async validateProjectAfterInstall(project, { reportError }) {
      if (!project.configuration.get(`enableConstraintsChecks`)) return;
        ...
        if (project.configuration.isCI) {
          for (const [workspace, workspaceErrors] of remainingErrors) {
            for (const error of workspaceErrors) {
              reportError(
                MessageName.CONSTRAINTS_CHECK_FAILED,
                `${formatUtils.pretty(project.configuration, workspace.anchoredLocator, formatUtils.Type.IDENT)}: ${error.text}`,
              );
            }
          }
        } else {
          reportError(
            MessageName.CONSTRAINTS_CHECK_FAILED,
            `Constraint check failed; run ${formatUtils.pretty(project.configuration, `yarn constraints`, formatUtils.Type.CODE)} for more details`,
          );
        }
      }
    },
