...
const yarnMigrationSymbol = Symbol.for(`yarnMigration`);

declare global {
  namespace NodeJS {
    interface Process {
      [yarnMigrationSymbol]?: boolean;
    }
  }
}

if (!process.env.YARN_ENABLE_MIGRATION || process[yarnMigrationSymbol]) {
  main();
} else {
  process[yarnMigrationSymbol] = true;
  module.exports = require(`./.yarn/ignore/migration/.pnp.cjs`);
}
