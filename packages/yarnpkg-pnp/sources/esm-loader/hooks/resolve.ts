...
export async function resolve(
  ...
  const issuer =
    parentURL && loaderUtils.tryParseURL(parentURL)?.protocol === `file:` ? fileURLToPath(parentURL) : process.cwd();
  ...
  if (!parentURL) loaderUtils.setEntrypointPath(fileURLToPath(resultURL));
