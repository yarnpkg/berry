...
function parseSpec<T>(spec: string, sourceParser: (source: string) => T) {
  ...
  if (protocol !== `patch:`) throw new Error(`Invalid patch range`);
  if (source === null) throw new Error(`Patch locators must explicitly define their source`);

  const patchPaths = selector ? selector.split(/&/).map((path) => npath.toPortablePath(path)) : [];
...
export function ensureUnpatchedDescriptor(descriptor: Descriptor) {
  if (!isPatchDescriptor(descriptor)) return descriptor;
...
export function ensureUnpatchedLocator(locator: Locator) {
  if (!isPatchLocator(locator)) return locator;
...
function makeSpec<T>(
  ...
  sourceStringifier: (source: T) => string,
) {
  const parentLocatorSpread =
    parentLocator !== null ? { locator: structUtils.stringifyLocator(parentLocator) } : ({} as {});

  const sourceVersionSpread = typeof sourceVersion !== `undefined` ? { version: sourceVersion } : ({} as {});

  const patchHashSpread = typeof patchHash !== `undefined` ? { hash: patchHash } : ({} as {});
...
export function makeDescriptor(
  ident: Ident,
  {
    ...
    sourceDescriptor,
    patchPaths,
  }: Pick<ReturnType<typeof parseDescriptor>, `parentLocator` | `sourceDescriptor` | `patchPaths`>,
) {
  return structUtils.makeDescriptor(
    ident,
    makeSpec({ parentLocator, sourceItem: sourceDescriptor, patchPaths }, structUtils.stringifyDescriptor),
  );
}
...
export function makeLocator(
  ident: Ident,
  {
    ...
    patchPaths,
    patchHash,
  }: Omit<ReturnType<typeof parseLocator>, "sourceLocator" | "sourceVersion"> & {
  ...
  return structUtils.makeLocator(
    ident,
    makeSpec(
      { parentLocator, sourceItem: sourcePackage, sourceVersion: sourcePackage.version, patchPaths, patchHash },
...
function visitPatchPath<T>(
  { onAbsolute, onRelative, onProject, onBuiltin }: VisitPatchPathOptions<T>,
  patchPath: PortablePath,
) {
  const flagIndex = patchPath.lastIndexOf(`!`);
  if (flagIndex !== -1) patchPath = patchPath.slice(flagIndex + 1) as PortablePath;

  const builtinMatch = patchPath.match(BUILTIN_REGEXP);
  if (builtinMatch !== null) return onBuiltin(builtinMatch[1]);

  if (patchPath.startsWith(`~/`)) return onProject(patchPath.slice(2) as PortablePath);
...
export function extractPatchFlags(patchPath: PortablePath) {
  ...
  const flags = flagIndex !== -1 ? new Set(patchPath.slice(0, flagIndex).split(/!/)) : new Set();
...
export async function loadPatchFiles(
  parentLocator: Locator | null,
  patchPaths: Array<PortablePath>,
  opts: FetchOptions,
) {
  ...
  const effectiveParentFetch =
    parentFetch && parentFetch.localPath
      ? {
          packageFs: new CwdFS(PortablePath.root),
          prefixPath: ppath.relative(PortablePath.root, parentFetch.localPath),
        }
      : parentFetch;

  // Discard the parent fs unless we really need it to access the files
  if (parentFetch && parentFetch !== effectiveParentFetch && parentFetch.releaseFs) parentFetch.releaseFs();
        ...
        const source = await visitPatchPath(
          {
            ...
            onRelative: async (patchPath) => {
              ...
              return await effectiveParentFetch.packageFs.readFilePromise(
                ppath.join(effectiveParentFetch.prefixPath, patchPath),
                `utf8`,
              );
            },
            ...
            onBuiltin: async (name) => {
              return await opts.project.configuration.firstHook(
                (hooks: PatchHooks) => {
  ...
  for (const spec of patchFiles) if (typeof spec.source === `string`) spec.source = spec.source.replace(/\r\n?/g, `\n`);
...
export async function diffFolders(folderA: PortablePath, folderB: PortablePath) {
  ...
  const normalizePath = folderAN.startsWith(`/`) ? (p: NativePath) => p.slice(1) : (p: NativePath) => p;
