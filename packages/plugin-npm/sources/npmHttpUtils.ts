 ...
 */
export async function handleInvalidAuthenticationError(
  error: any,
  {
    attemptedAs,
    registry,
  ...
  if (isOtpError(error)) throw new ReportError(MessageName.AUTHENTICATION_INVALID, `Invalid OTP token`);

  if (error.originalError?.name === `HTTPError` && error.originalError?.response.statusCode === 401) {
    throw new ReportError(
      MessageName.AUTHENTICATION_INVALID,
      `Invalid authentication (${typeof attemptedAs !== `string` ? `as ${await whoami(registry, headers, { configuration })}` : `attempted as ${attemptedAs}`})`,
    );
  }
}
...
export function customPackageError(error: httpUtils.RequestError, configuration: Configuration) {
  ...
  if (statusCode === 404) return `Package not found`;
...
export async function getPackageMetadata(
  ident: Ident,
  { cache, project, registry, headers, version, ...rest }: GetPackageMetadataOptions,
        ...
        if (configuration.get(`enableOfflineMode`)) {
          ...
          if (cache) {
            ...
            if (deleted.has(latest)) {
              const allVersions = Object.keys(cached.metadata.versions).sort(semver.compare);

              let latestIndex = allVersions.indexOf(latest);
              while (deleted.has(allVersions[latestIndex]) && latestIndex >= 0) latestIndex -= 1;
    ...
    return await get(getIdentUrl(ident), {
      ...
      wrapNetworkRequest: async (executor) => async () => {
        ...
        if (response.statusCode === 304) {
          if (cached === null) throw new Error(`Assertion failed: cachedMetadata should not be null`);
...
function pickPackageMetadata(metadata: PackageMetadata): PackageMetadata {
  return {
    ...
    versions: Object.fromEntries(
      Object.entries(metadata.versions).map(([key, value]) => [key, pick(value, CACHED_FIELDS) as any]),
    ),
  };
}
...
export async function get(path: string, { configuration, headers, ident, authType, registry, ...rest }: Options) {
  ...
  if (auth) headers = { ...headers, authorization: auth };

  try {
    return await httpUtils.get(path.charAt(0) === `/` ? `${registry}${path}` : path, {
      configuration,
      headers,
      ...rest,
    });
...
export async function post(
  ...
  body: httpUtils.Body,
  {
    ...
    otp,
    ...rest
  }: Options & { attemptedAs?: string },
) {
  registry = normalizeRegistry(configuration, { ident, registry });

  const auth = await getAuthenticationHeader(registry, { authType, configuration, ident });
  if (auth) headers = { ...headers, authorization: auth };
  if (otp) headers = { ...headers, ...getOtpHeaders(otp) };
...
function normalizeRegistry(configuration: Configuration, { ident, registry }: Partial<RegistryOptions>): string {
  if (typeof registry === `undefined` && ident) return npmConfigUtils.getScopeRegistry(ident.scope, { configuration });
...
async function getAuthenticationHeader(
  registry: string,
  {
    ...
    configuration,
    ident,
  }: { authType?: AuthType; configuration: Configuration; ident: RegistryOptions["ident"] },
) {
  ...
  const header = await configuration.reduceHook(
    (hooks: Hooks) => {
  ...
  if (effectiveConfiguration.get(`npmAuthIdent`)) {
    ...
    if (npmAuthIdent.includes(`:`)) return `Basic ${Buffer.from(npmAuthIdent).toString(`base64`)}`;
...
async function askForOtp(error: any, { configuration }: { configuration: Configuration }) {
  ...
  if (notice) {
    await StreamReport.start(
      ...
      async (report) => {
        report.reportInfo(
          MessageName.UNNAMED,
          notice.replace(/(https?:\/\/\S+)/g, formatUtils.pretty(configuration, `$1`, formatUtils.Type.URL)),
        );
            ...
            if (openNow) {
              if (!(await nodeUtils.openUrl(autoOpen[1]))) {
                ...
                report.reportWarning(
                  MessageName.UNNAMED,
                  `We failed to automatically open the url; you'll have to open it yourself in your browser of choice.`,
                );
              }
            }
          }
  ...
  if (process.env.YARN_IS_TEST_ENV) return process.env.YARN_INJECT_NPM_2FA_TOKEN || ``;
...
function isOtpError(error: any) {
  if (error.originalError?.name !== `HTTPError`) return false;

  try {
    const authMethods = error.originalError?.response.headers[`www-authenticate`]
      .split(/,\s*/)
