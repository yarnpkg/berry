...
export default class NewPluginCommand extends Command {
  ...
  async execute() {
    ...
    await xfs.writeFilePromise(ppath.join(target, `sources` as Filename, `index.ts` as Filename), [
    ...
    await xfs.writeFilePromise(ppath.join(target, `.gitignore` as Filename), `bundles/\n`);
