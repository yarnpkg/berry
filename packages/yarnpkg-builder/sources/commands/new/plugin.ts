...
export default class NewPluginCommand extends Command {
  ...
  static usage: Usage = Command.Usage({
    ...
    `,
    examples: [[`Create a new plugin`, `$0 new plugin yarn-plugin-hello-world`]],
  });
  ...
  async execute() {
    ...
    await xfs.writeFilePromise(
      ppath.join(target, `sources`, `index.ts`),
      [
    ...
    this.context.stdout.write(
      `Scaffolding done! Just go into ${chalk.magenta(npath.fromPortablePath(target))} and run ${chalk.cyan(`yarn && yarn build`)} 🙂\n`,
    );
  }
}
