let OSRelease: Array<string> | undefined;
...
let warned = false;
function warnOnDeactivatedColors(env: NodeJS.ProcessEnv) {
  if (warned)
    return;
  let name = ``;
...
function getColorDepth(env: NodeJS.ProcessEnv = process.env) {
  ...
  if (env.TERM) {
    ...
    if (Object.hasOwn(TERM_ENVS, termEnv))
      return TERM_ENVS[termEnv as keyof typeof TERM_ENVS];
