...
export function extractBuildRequest(
  pkg: Package,
  requirements: ExtractBuildScriptDataRequirements,
    ...
    return {
      ...
      explain: (report) =>
        report.reportWarningOnce(
          ...
          `${structUtils.prettyLocator(configuration, pkg)} lists build scripts, but is referenced through a soft link. Soft links don't support build scripts, so they'll be ignored.`,
        ),
    };
    ...
    return {
      ...
      explain: (report) =>
        report.reportInfoOnce(
          ...
          `${structUtils.prettyLocator(configuration, pkg)} lists build scripts, but its build has been explicitly disabled through configuration.`,
        ),
    };
    ...
    return {
      ...
      explain: (report) =>
        report.reportWarningOnce(
          ...
          `${structUtils.prettyLocator(configuration, pkg)} lists build scripts, but all build scripts have been disabled.`,
        ),
    };
  ...
  if (!checkManifestCompatibility(pkg))
    return {
      ...
      explain: (report) =>
        report.reportWarningOnce(
          MessageName.INCOMPATIBLE_ARCHITECTURE,
          `${structUtils.prettyLocator(configuration, pkg)} The ${nodeUtils.getArchitectureName()} architecture is incompatible with this package, build skipped.`,
        ),
    };
