...
export class ExecFetcher implements Fetcher {
  ...
  getLocalPath(locator: Locator, opts: FetchOptions) {
    ...
    if (posix.isAbsolute(execPath)) return execPath;
  ...
  private async fetchFromDisk(locator: Locator, opts: FetchOptions) {
    ...
    const generatorPath = posix.resolve(
      posix.resolve(generatorFs.getRealPath(), effectiveParentFetch.prefixPath),
      execPath,
    );
    ...
    if (!xfs.existsSync(`${cwd}/build`)) throw new Error(`The script should have generated a build directory`);
  ...
  private async generatePackage(locator: Locator, generatorPath: string, opts: FetchOptions) {
    ...
    stdout.write(
      `# This file contains the result of Yarn generating a package (${structUtils.stringifyLocator(locator)})\n`,
    );
    stdout.write(`\n`);

    const {code} = await execUtils.pipevp(
      process.execPath,
      [NodeFS.fromPortablePath(generatorPath), structUtils.stringifyIdent(locator)],
