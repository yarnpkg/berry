...
export function makeSpec({
  ...
  const generatorHashSpread = typeof generatorHash !== `undefined` ? { hash: generatorHash } : ({} as {});
...
export function makeLocator(
  ident: Ident,
  { parentLocator, path, generatorHash, protocol }: Parameters<typeof makeSpec>[number],
): Locator {
