...
export class PnpmLinker implements Linker {
  ...
  async findPackageLocation(locator: Locator, opts: LinkOptions) {
    if (!this.isEnabled(opts)) throw new Error(`Assertion failed: Expected the pnpm linker to be enabled`);
    ...
    if (typeof packagePaths === `undefined`)
      throw new UsageError(
        `Couldn't find ${structUtils.prettyLocator(opts.project.configuration, locator)} in the currently installed pnpm map - running an install might help`,
      );
...
class PnpmInstaller implements Installer {
  ...
  async installPackageHard(pkg: Package, fetchResult: FetchResult, api: InstallPackageExtraApi) {
    ...
    const buildRequest = jsInstallUtils.extractBuildRequest(pkg, buildConfig, dependencyMeta, {
  ...
  async attachInternalDependencies(locator: Locator, dependencies: Array<[Descriptor, Locator]>) {
    if (this.opts.project.configuration.get(`nodeLinker`) !== `pnpm`) return;

    // We don't install those packages at all, because they can't be used anyway
    if (!isPnpmVirtualCompatible(locator, { project: this.opts.project })) return;
    ...
    this.asyncActions.reduce(locator.locatorHash, async (action) => {
        ...
        concurrentPromises.push(
          Promise.resolve().then(async () => {
            ...
            if (
              ...
              this.opts.project.configuration.get(`winLinkType`) === WindowsLinkType.JUNCTIONS
            ) {
      ...
      for (const [descriptor, dependency] of dependencies) {
        if (descriptor.identHash === locator.identHash) hasExplicitSelfDependency = true;
...
async function cleanNodeModules(nmPath: PortablePath, extraneous: Map<PortablePath, DirentNoPath>) {
  ...
  return Promise.all(removeNamePromises).then(() =>
    Promise.all([...scopesToRemove].map((scope) => removeIfEmpty(ppath.join(nmPath, scope)))),
  ) as Promise<void>;
}
