...
type InstallState = {locatorMap: NodeModulesLocatorMap, locationTree: LocationTree, binSymlinks: BinSymlinkMap, hardlinks: boolean};
...
async function writeInstallState(project: Project, locatorMap: NodeModulesLocatorMap, binSymlinks: BinSymlinkMap, hardlinks: boolean) {
  let locatorState = ``;
  ...
  locatorState += `  hardlinks: ${hardlinks}\n`;
...
async function findInstallState(project: Project, {unrollAliases = false}: {unrollAliases?: boolean} = {}): Promise<InstallState | null> {
  ...
  const hardlinks = locatorState.__metadata.hardlinks === `true`;
  ...
  return {locatorMap, binSymlinks, locationTree: buildLocationTree(locatorMap, {skipPrefix: project.cwd}), hardlinks};
}
...
async function persistNodeModules(preinstallState: InstallState, installState: NodeModulesLocatorMap, {baseFs, project, report, loadManifest}: {project: Project, baseFs: FakeFS<PortablePath>, report: Report, loadManifest: LoadManifest}) {
  ...
  const cloneModule = async (srcDir: PortablePath, dstDir: PortablePath, options: { useHardlinks: boolean, innerLoop?: boolean }) => {
    const promise: Promise<any> = (async () => {
      const cloneDir = async (srcDir: PortablePath, dstDir: PortablePath, options: { useHardlinks: boolean, innerLoop?: boolean }) => {
        try {
          if (!options.innerLoop)
            await xfs.mkdirPromise(dstDir, {recursive: true});
  ...
  try {
    ...
    for (const entry of addList) {
      ...
      if (entry.linkType !== LinkType.SOFT && entry.dstDir !== persistedDir) {
        await cloneModule(persistedDir, entry.dstDir, {useHardlinks});
      }
    }
    ...
    await writeInstallState(project, installState, binSymlinks, useHardlinks);
  } finally {
