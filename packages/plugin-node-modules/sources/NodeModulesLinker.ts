...
import {VirtualFS, ZipOpenFS, xfs, FakeFS, NativePath, Stats}                                  from '@yarnpkg/fslib';
...
class NodeModulesInstaller implements Installer {
  ...
  async finalizeInstall() {
    ...
    if (preinstallState === null || nmMode !== preinstallState.nmMode) {
      ...
      preinstallState = {locatorMap: new Map(), binSymlinks: new Map(), locationTree: new Map(), contentFingerprints: {uniqueChecksums: new Map(), duplicateChecksums: new Set(), hashes: new Map()}, nmMode};
    }
...
async function writeInstallState(project: Project, locatorMap: NodeModulesLocatorMap, binSymlinks: BinSymlinkMap, nmMode: NodeModulesMode, contentFingerprints: ContentFingerprints) {
  let locatorState = ``;
    ...
    if (locator === topLevelLocator && binSymlinks.size > 0) {
      ...
      if (contentFingerprints.hashes.size > 0) {
        ...
        for (const [hash, dstPath] of contentFingerprints.hashes) {
          ...
          locatorState += `  - [${hash}, ${internalPath}]\n`;
        }
      }
    }
  }
...
async function findInstallState(project: Project, {unrollAliases = false}: {unrollAliases?: boolean} = {}): Promise<InstallState | null> {
  ...
  const contentFingerprints: ContentFingerprints = {uniqueChecksums: new Map(), duplicateChecksums: new Set(), hashes: new Map()};
  ...
  for (const [locatorStr, installRecord] of Object.entries(locatorState)) {
    ...
    if (installRecord.duplicateChecksums) {
      for (const checksum of installRecord.duplicateChecksums) {
        contentFingerprints.duplicateChecksums.add(checksum);
      }
    }

    if (installRecord.hashes) {
      for (const [hash, relativeLocation] of installRecord.hashes) {
        contentFingerprints.hashes.set(hash, ppath.join(rootPath, npath.toPortablePath(relativeLocation)));
      }
    }
  ...
  return {locatorMap, binSymlinks, locationTree: buildLocationTree(locatorMap, {skipPrefix: project.cwd}), nmMode, contentFingerprints};
}
...
const CONCURRENT_OPERATION_LIMIT = 8;
 ...
 * only for the files that have the same CRC inside zip archives in the cache, if files have different CRC no need to compute
 * MD5 since, they will be different too.
 */
...
const copyPromise = async (dstDir: PortablePath, srcDir: PortablePath, {baseFs, nmMode, contentFingerprints, innerLoop}: {baseFs: FakeFS<PortablePath>, nmMode: NodeModulesMode, contentFingerprints: ContentFingerprints, innerLoop?: boolean}) => {
  ...
  const copy = async (dstPath: PortablePath, srcPath: PortablePath, srcType: fs.Dirent) => {
    if (srcType.isFile()) {
      ...
      await copyFilePromise({srcPath, dstPath, srcStat, baseFs, nmMode, contentFingerprints});
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (entry.name !== NODE_MODULES || innerLoop) {
        await copyPromise(dstPath, srcPath, {baseFs, nmMode, contentFingerprints, innerLoop: true});
      }
    } else {
...
async function persistNodeModules(preinstallState: InstallState, installState: NodeModulesLocatorMap, {baseFs, project, report, loadManifest}: {project: Project, baseFs: FakeFS<PortablePath>, report: Report, loadManifest: LoadManifest}) {
  ...
  const addModule = async ({srcDir, dstDir, linkType, nmMode, contentFingerprints}: {srcDir: PortablePath, dstDir: PortablePath, linkType: LinkType, nmMode: NodeModulesMode, contentFingerprints: ContentFingerprints}) => {
    const promise: Promise<any> = (async () => {
      try {
        if (linkType === LinkType.SOFT) {
          ...
          await copyPromise(dstDir, srcDir, {baseFs, nmMode, contentFingerprints});
        }
      } catch (e) {
  ...
  try {
    const prevFingerprints = preinstallState.contentFingerprints;
    const contentFingerprints: ContentFingerprints = {
      uniqueChecksums: new Map(prevFingerprints.uniqueChecksums),
      duplicateChecksums: new Set(prevFingerprints.duplicateChecksums),
      hashes: new Map(prevFingerprints.hashes),
    };
    ...
    for (const entry of addList) {
      if (entry.linkType === LinkType.SOFT || !persistedLocations.has(entry.srcDir)) {
        ...
        await addModule({...entry, nmMode, contentFingerprints});
      }
    }
    ...
    await writeInstallState(project, installState, binSymlinks, nmMode, contentFingerprints);
  } finally {
